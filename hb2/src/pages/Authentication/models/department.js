...
const updateTre = (ctree, cnode, type, isRoot=false) => {
  ...
  if (isRoot) {
    ...
    // else if(type === 3){
    //   tree = tree.filter((item) => item.id = node.id)
    //   return undefined
    // }
  }
...
export default {
  ...
  effects: {
    *add({ payload }, { put }) {
      const {item, isRoot} = payload
      tempIndexAdd += 1
      item.id = tempIndexAdd
      item.code = tempIndexAdd.toString()
      yield put({
        type: 'updateTree',
        payload: { item, type: 1, isRoot}
      });
    },
    *update({ payload }, { put }) {
      yield put({
  ...
  reducers: {
    ...
    updateTree (state, { payload }) {
      ...
      const params = {}
      updateTre(state.group, item, type, isRoot);
      if (type === 3) {
