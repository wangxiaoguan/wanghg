...
@connect(({ roleOrgModal, loading }) => ({
  roleOrgModal,
  loading: loading.effects['roleOrgModal/fetch'],
}))
@Form.create()
class roleOrgModal extends PureComponent {
  state = {
    formValues: {},
    selectedRowKeys: [],
  };
  ...
  okHandle = () => {
    const { handleOk, dispatch, roleId } = this.props;
    ...
    if (selectedRowKeys.length === 0) {
      ...
      dispatch({
        type: 'roleOrgModal/bindOrg',
        payload: { roleId, orgIds: selectedRowKeys },
        callback: () => {
