...
import { Card, Form, Spin, Button, Switch, message } from 'antd';
...
const EDIT_HASH = '#/EnglishSite/InvestigationList/InvestigationEdit';
...
@connect(({ loading }) => (
  {
    loadingDelete: Boolean(loading.effects['investigation/remove']),
    loadingStick: Boolean(loading.effects['investigation/stick']),
  }
))
class InvestigationList extends Component<any, any> {
      ...
      render: (_, record) => {
        return (
          <span className='controlsContainer'>
            ...
            <Switch
              loading={this.props.loadingStick}
              onChange={(check) => {
                this.props.dispatch(
                  {
                    type: 'investigation/stick',
                    payLoad: {
                      id: record.id,
                      isStick: check ? 1 : 0
                    },
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    return `/services/wto/wtoinvestment/list/${current}/${pageSize}${createSearchString(values)}`;
  }
...
@connect(({ loading }) => (
  {
    loadingDelete: Boolean(loading.effects['investigation/remove']),
    loadingCheckStatus: Boolean(loading.effects['investigation/updateCheckStatus']),
  }
))
class SearchForm extends Component<any>  {
  ...
  updateCheckStatus(idList: string[], isCheck: boolean) {
    ...
    this.props.dispatch(
      {
        type: 'investigation/updateCheckStatus',
        payLoad: params,
        callBack: () => {
          this.props.refresh();
        }
