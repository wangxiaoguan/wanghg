...
const EDIT_HASH = '#/wtoDictionary/ICSCodeList/ICSCodeEdit';
/**
 * ICS码管理
 */
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['ICSCode/remove']),
}))
class ICSCodeList extends Component<IDispatchInterface, any> {
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    ...
    if(flag){
      return `services/wto/wtoicsinfo/list/${current}/${pageSize}/${createSearchString(values)}`;
    }else{
      return `services/wto/wtoicsinfo/list/${current}/${pageSize}`
    }
  }
