...
import RecallNewsEnum from '@/Enums/RecallNewsEnum';
...
const EDIT_HASH = '#/defectProductContent/RecallAdvert/RecallAdvertEdit';
/**
 * 动态新闻
 */
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['faultyRecall/remove']),
}))
class SearchViewTemplete extends Component<IDispatchInterface, any> {
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    const data = { publishColumn: '2', deleteStatus: 1, ...values }
...
class SearchForm extends Component<any>  {
  batchUpdate = (selectedRowKeys, status) => {
    ...
    this.props.dispatch(
      {
        type: 'faultyRecall/updateBatchCheckStatus',
        payLoad: arr,
        callBack: () => {
          this.props.refresh();
        }
    ...
    const FORM_ITEMS = [
      ...
      },
      {
        ...
        content: getFieldDecorator('type')(<HSelect>
          {
            createSelectOptions(RecallNewsEnum.ALL_LIST, RecallNewsEnum.toString)
          }
        </HSelect>),
      },
      {
