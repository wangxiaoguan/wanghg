...
const EDIT_HASH = '#/defectProductContent/Knowledge/KnowledgeEdit';
/**
 * 动态新闻
 */
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['faultyKnowledge/remove']),
}))
class SearchViewTemplete extends Component<IDispatchInterface, any> {
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    const data = { publishColumn: '4', deleteStatus: 1, ...values }
...
class SearchForm extends Component<any>  {
  batchUpdate = (selectedRowKeys, status) => {
    ...
    this.props.dispatch(
      {
        type: 'faultyKnowledge/updateBatchCheckStatus',
        payLoad: arr,
        callBack: () => {
          this.props.refresh();
        }
