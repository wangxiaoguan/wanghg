...
import LawsNewsEnum from '@/Enums/LawsNewsEnum';
...
const EDIT_HASH = '#/defectProductContent/Laws/LawsEdit';
/**
 * 动态新闻
 */
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['faultyLaws/remove']),
}))
class SearchViewTemplete extends Component<IDispatchInterface, any> {
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    const data = { publishColumn: '5', deleteStatus: 1, ...values }
...
class SearchForm extends Component<any>  {
  batchUpdate = (selectedRowKeys, status) => {
    ...
    this.props.dispatch(
      {
        type: 'faultyLaws/updateBatchCheckStatus',
        payLoad: arr,
        callBack: () => {
          this.props.refresh();
        }
