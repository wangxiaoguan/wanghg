...
const EDIT_HASH = '#/defectProductContent/NewsList/NewsEdit';
/**
 * 动态新闻
 */
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['faultyNews/remove']),
}))
class SearchViewTemplete extends Component<IDispatchInterface, any> {
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    const data = { publishColumn: 1, deleteStatus: 1, ...values }
...
class SearchForm extends Component<any>  {
  batchUpdate = (selectedRowKeys, status) => {
    ...
    this.props.dispatch(
      {
        type: 'faultyNews/updateBatchCheckStatus',
        payLoad: arr,
        callBack: () => {
          this.props.refresh();
        }
