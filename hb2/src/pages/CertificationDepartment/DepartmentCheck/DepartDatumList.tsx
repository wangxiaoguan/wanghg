...
class DepartDatumList extends Component<IDispatchInterface, any> {
  ...
  private COLUMNS: any[] = [
    ...
    },
    {
      ...
      render: (_, record) => {
        if(record.fileId){
          return <a href={DOWNLOAD_API(record.fileId)} target='_blank'>下载</a>
        }else{
          return <span style={{color:'#666'}}>下载</span>
        }
      }
    },];
  ...
  searchCreater = (values: any, pageSize: number, current: number) => {
    return `/services/exam/fhexamdatum/selectAll2/${current}/${pageSize}${createSearchString(values)}`;
  }
...
class SearchForm extends Component<any, any>  {
  ...
  batchUpdate = (selectedRowKeys, checkStatus, comments) => {
    ...
    this.props.dispatch(
      {
        type: 'DepartDatumList/updateCheckStatus',
        payLoad: params,
        callBack: () => {
          this.props.refresh();
          this.modal.close()
...
export default DepartDatumList;