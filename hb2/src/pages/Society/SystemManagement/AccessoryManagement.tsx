...
const EDIT_HASH = '#/society/rawData/accessoryManagement';
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['accessoryManagement/remove']),
}))
class AccessoryManagement extends Component<any, any> {
  private COLUMNS = [
    {
      title: '通报号',
  ...
  remove = (id) => {
    this.props.dispatch(
      {
        type: 'accessoryManagement/remove',
        payLoad: id,
        callBack: () => {
          this.table.refresh();
        }
...
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['accessoryManagement/remove']),
  loadingCheckStatus: Boolean(loading.effects['accessoryManagement/updateCheckStatus']),
}))
class SearchForm extends Component<any, any> {
  ...
  updateCheckStatus(idList: string[], isCheck: boolean) {
    ...
    this.props.dispatch(
      {
        type: 'accessoryManagement/updateCheckStatus',
        payLoad: params,
        callBack: () => {
          this.props.refresh();
        }
