...
const EDIT_HASH = '#/society/rawData/systemLog';
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['systemLog/remove']),
}))
class SystemLog extends Component<any, any> {
  private COLUMNS = [
    {
      title: '通报号',
...
@connect(({ loading }) => ({
  loadingDelete: Boolean(loading.effects['systemLog/remove']),
  loadingCheckStatus: Boolean(loading.effects['systemLog/updateCheckStatus']),
}))
class SearchForm extends Component<any, any> {
  ...
  updateCheckStatus(idList: string[], isCheck: boolean) {
    ...
    this.props.dispatch(
      {
        type: 'systemLog/updateCheckStatus',
        payLoad: params,
        callBack: () => {
          this.props.refresh();
        }
