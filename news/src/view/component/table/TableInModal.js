...
import ServiceApi, { masterUrl } from '../../content/apiprefix';
...
class TableInModal extends Component{
  ...
  getData=(url)=>{
    console.log("tableInModal中的url",url);
     getService(ServiceApi+url,data=>{
       console.log("返回的dataServiceApi+url",ServiceApi+url);
  ...
  handleOk=(e)=>{
    ...
    postService(ServiceApi + `${this.state.addUrl}`,values, data => {
      if (data.retCode == 1) {
