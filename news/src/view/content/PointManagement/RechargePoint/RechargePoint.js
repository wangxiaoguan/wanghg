...
export default class RechargePoint extends Component {
  ...
  componentDidMount(){
    ...
    //获取积分充值配置列表初始数据
    getService(ServiceApi + 'services/system/treasureRecharge/list/1/10',data=>{
  ...
  deleteData = ()=>{
    ...
    postService(ServiceApi+'services/system/treasureRecharge/remove/TreasureRecharge', body, data => {
      if(data.retCode === 1){
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(ServiceApi + `services/system/treasureRecharge/list/${this.state.currentPage}/${this.state.pageSize}`, data => {
      if(data.retCode === 1){
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) =>{
    getService(ServiceApi + `services/system/treasureRecharge/list/${current}/${pageSize}`,data =>{
