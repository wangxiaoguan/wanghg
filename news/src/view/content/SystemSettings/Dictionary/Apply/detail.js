...
export default class ApplyDetail extends Component {
  ...
  componentDidMount(){
    ...
    // });
    // getService(ServiceApi + 'services/lookup/init/enrollFieldRequried', data => {
    //   console.log('loockup', data);
    // });
    this.setState({
      FieldTypeOption: [{
    ...
    if (this.state.isEdit) {
      getService(ServiceApi+`/services/system/dictionary/enrollField/getDetailById/${this.state.id}`,data=>{
        if (data.retCode===1) {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      if (!err) {
        ...
        if (this.state.isEdit) {
          ...
          postService(ServiceApi + 'services/system/dictionary/enrollField/update', body, data => {
            if (data.retCode === 1) {
          ...
          postService(ServiceApi + 'services/system/dictionary/enrollField/add', body, data => {
            if (data.retCode === 1) {
