...
export default class List extends  Component {
  ...
  dealData=()=>{
    //获取榜单的详细信息
    getService(ServiceApi+`services/partybuilding/taskStateTxUI/boardDetai/get/1/10/${this.state.queryRange}`,(data)=>{
      if(data.retCode===1){
  ...
  onPageChange = (current, pageSize) =>{
    getService(ServiceApi + `services/partybuilding/taskStateTxUI/boardDetai/get/${current}/${pageSize}/${this.state.queryRange}`,data =>{
      if(data.retCode === 1){
