...
class OrderInformation extends Component {
  ...
  componentDidMount(){
    getService(ServiceApi + `services/lookup/init/orderState`, data => {
      data.map((item,index) => {
        item.key = item.code
    ...
    getService(ServiceApi + `services/lookup/init/orderAddress`, data => {
      data.map((item,index) => {
        item.key = item.code
        item.value = item.desp
      })
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        if(this.state.value == 1){
          postService(ServiceApi + `services/activity/orderActivity/order/email`,data,data => {
              if(data.retCode == 1){
           ...
           data.operateType = 1,
           data.queryType = 'post',
           data.queryUrl = ServiceApi + `services/activity/orderActivity/order/email`,
        /*   this.props.keys.map((item) => {
           ...
           postService(ServiceApi + `services/automation/job/add`,data,data => {
              if(data.retCode == 1){
               /* Message.success("新增成功!")
                location.hash='EventManagement/Order/Commodity'*/
              }else{
