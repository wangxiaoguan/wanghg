...
export default class TopicAdd extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.isEdit) {
      getService(
        ServiceApi + `services/activity/voteActivity/topic/info/${this.state.id}`,
        data => {
          if (data.retCode === 1) {
            let value = data.root;
            ...
            let temp = option && option.map((item,index) => {
              return item = { response: { entity: [{ filePath: item.url }] }, describe: item.desp1, id:index };
            });
            this.setState(
              {
                answerGroup,
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (!err) {
        ...
        if (this.state.isEdit) {
          ...
          postService(
            ServiceApi + 'services/activity/voteActivity/topic/edit',
            body,
            data => {
              if (data.retCode === 1) {
          ...
          postService(
            ServiceApi + 'services/activity/voteActivity/topic/add',
            body,
            data => {
              if (data.retCode === 1) {
  ...
  handleChange(e, index,id) {
    ...
    if(this.state.isEdit){
      ...
      if(id===0){
        ...
        inputValue[index] = { content: e.target.value, id:inputValue[index].id , blank };
      }
    }else{
      console.log('aaa', answerGroup);
      let blank =
