...
export default class AddCustomQuestions extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.isEdit) {
      getService(ServiceApi + `services/activity/examActivity/getActivityET/${this.state.topicId}`, data => {
        if (data.retCode === 1) {
          let value = data.root;
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      if (!err) {
        ...
        if (this.state.isEdit) {    
          ...
          postService(ServiceApi + 'services/activity/examActivity/updateActivityET', body, data => {
            if (data.retCode === 1) {
          ...
          postService(ServiceApi + 'services/activity/examActivity/insertActivityET', body, data => {
            if (data.retCode === 1) {
