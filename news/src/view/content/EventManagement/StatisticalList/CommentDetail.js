...
class CommentDetail extends Component {
  ...
  componentDidMount(){
    ...
    if(this.state.commentId){
      getService(ServiceApi + `services/activity/comment/info/${this.state.targetType}/${this.state.commentId}`, data => {
        if (data.retCode === 1) {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      console.log("**********",body);
      postService(ServiceApi + 'services/activity/comment/reply', body, data => {
        if (data.retCode == 1) {
          history.back()
        } else {
