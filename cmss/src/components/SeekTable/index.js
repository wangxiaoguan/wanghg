...
class SeekTable extends Component {
  ...
  handleOk = () => {
    const { selectSeek } = this.state;
    const { setSeek, cancelTable } = this.props;
    if (selectSeek.length === 0) {
      message.warning('请选择资讯');
      return;
    }
    cancelTable();
    setSeek(selectSeek, 'seekList');
  };
