...
export default class ImportWrapper extends Component {
  handleClick=(e,path)=>{
    ...
    exportExcelService(API_PREFIX+path, null, this.props.title); 
  }
  beforeUpload = (file) => {
  ...
  handleChange = (info) => {
    ...
    if (info.file.response) {
      if (info.file.response.retCode == 1) {
        ...
        this.props.getData(API_PREFIX +`services/activity/topic/list/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
  ...
  render(){
    return (<Row>
      ...
      <Col span={6}>
        <Upload
          name="file"
          action={API_PREFIX + this.props.uploadUrl}
