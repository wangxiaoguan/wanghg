...
export default class FormWrapper extends Component {
  ...
  setInitialValue = (data) => {
    ...
    if(data.examid){
      getService(API_PREFIX + 'services/activity/examActivity/list/1/10?Q=id_S_LK='+data.examid, data => {
        if (data.retCode === 1) {
  ...
  componentDidMount() {
    ...
    if (this.props.initialValue) {
      ...
      console.log('2222')
      console.log(this.props.leaveData[this.props.belonged])
    ...
    getService(API_PREFIX + 'services/system/organization/organizationList/get', data => {
      if (data.retCode === 1) {
        ...
        let type = this.props.form.getFieldValue('belongOrgType');
        if (type === '1') {
    ...
    getService(API_PREFIX + 'services/system/partyOrganization/partyOrganizationList/get', data => {
      if (data.retCode === 1) {
        ...
        if (type === '2') {
          this.setState({
            partyOrganization: partyOrganizationData,
            belongsData: partyOrganizationData,
          });
        } else {
    ...
    if (this.state.numStatus) {
      ...
      if (this.props.selectRowsData.length != 0) {
        ...
        this.setState({
          ...
          console.log("num", this.state.shoppingId)
        })
  ...
  handleTreasureChange = (value, key) => {
    //当key值为  奖励积分时，才需要处理
    console.log('handleTreasureChange', value, key);
    if (key == 'treasure') {
      if (value > 0) {
        console.log('handleTreasureChange2', value, key);
        this.setState({ isValid: true }, () => console.log('handleTreasureChange', this.state.isValid));
      }
    }
  }
  ...
  deleteShopping = (item) => {
    ...
    console.log("index", shoppingNum)
    this.setState({
      shoppingNum: shoppingNum,
      shoppingId: shoppingId
    }, () => {
      console.log("sum", this.state.shoppingId)
    })
  }
  render() {
    console.log("shoppingNum", this.state.selectRowsData)
    ...
    return <div>{content}
      <Modal
        ...
        destroyOnClose
      >
      {API_CHOOSE_SERVICE==1?<img src={API_FILE_VIEW + this.state.imgValue} style={{ width: '520px', height: '300px' }} />:
