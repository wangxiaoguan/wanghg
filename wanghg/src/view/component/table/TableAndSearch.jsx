 ...
 */
    ...
    data&&data.map((item, index) => {
      ...
      if(item.selectCount){
        ...
        // if(item.singleSize===undefined){
        //   item.singleSize=0;
        //   item.multipleSize=0;
        // }else{
        //   if(item.singleSize===0&&item.multipleSize===0){
        //     item.maxsingleSize=item.selectCount;
        //     item.maxmultipleSize=item.selectCount;
        //   }else{
        //     if(item.singleSize===0){
        //       item.maxmultipleSize=item.selectCount;
        //     }else{
        //       if(item.singleSize<item.selectCount){
        //         item.maxsingleSize=item.selectCount-item.multipleSize;
        //         item.maxmultipleSize=item.selectCount-item.singleSize;
        //       }else if(item.singleSize===item.selectCount){
        //         item.maxsingleSize=item.selectCount;
        //         item.maxmultipleSize=0;
        //       }
        //     }
  
        //     if(item.multipleSize===0){
        //       item.maxsingleSize=item.selectCount;
        //     }else{
        //       if(item.multipleSize<item.selectCount){
        //         item.maxmultipleSize=item.selectCount-item.singleSize;
        //         item.maxsingleSize=item.selectCount-item.multipleSize;
        //       }else if(item.multipleSize===item.selectCount){
        //         item.maxmultipleSize=item.selectCount;
    ...
    const deleteBtn = this.props.deleteBtn ? <Popconfirm title="确定删除所选项吗？" onConfirm={this.deleteData.bind(null, this.props.deleteBtn.url, this.props.deleteBtn.field,this.props.deleteBtn.special)} okText="确定" cancelText="取消">
