...
class NormalLoginForm extends React.Component {
  ...
  componentWillMount(){
  	getService(API_PREFIX + `services/system/edition/list/noPage`,data => {
      if (data.retCode == 1) {
        console.log(data)
        this.setState({
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(this.props.flag){
          ...
          postService(API_PREFIX + `services/system/company/update`,values, data => {
            if (data.retCode == 1) {
          ...
          postService(API_PREFIX + `services/system/company/add`,values, data => {
            if (data.retCode == 1) {
  ...
  handleValidMobile= (rule, value, callback) => {
    if(value == ''||value==undefined){
      ...
      getService(API_PREFIX + `services/system/verify/checkUnique/PlatformAdministrator-company?${query}`, data => {
        if(data.retCode==0){
