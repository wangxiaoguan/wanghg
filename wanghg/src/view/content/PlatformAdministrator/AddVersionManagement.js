...
class AddVersionManagement extends Component {
  ...
  componentWillMount(){
     if(this.state.flag == 'true'){
      ...
      getService(API_PREFIX + `services/system/edition/permission/`+ this.state.roleId,data => {
        if (data.retCode == 1) {
            const arr5 = [];
            const arr6= [];
...
/*    let param = GetQueryString(location.hash,['flag','roleId','isEdit']);
    ...
    getService(API_PREFIX + `services/system/edition/permission/menu/all`,data => {
      if (data.retCode == 1) {
      ...
      if (!err) {
          ...
          if(data != undefined){
             if(this.state.isEdit){
                ...
                postService(API_PREFIX + `services/system/edition/update`,data,data => {
                  if (data.retCode == 1) {
                    this.setState({
                      operation:[],
                      sum:[]
                ...
                postService(API_PREFIX + `services/system/edition/add`,data,data => {
                  if (data.retCode == 1) {
                    this.setState({
                      operation:[],
                      sum:[]
 ...
 /* handleSubmit(){
  ...
  onTabClick = (key) =>{
    ...
    this.setState({
      ...
      if(key == '2'){
        postService(API_PREFIX + `services/system/edition/permission/operation`,{menuPermission:this.state.operationPermissions},data => {
          if (data.retCode == 1) {
            this.setState({
  ...
  handleConfirmPassword = (rule, value, callback) => {
      ...
      if(this.state.isEdit){
        if(value == undefined||value == ''){
          ...
          postService(API_PREFIX + `services/system/edition/check/name`,{"id":this.state.roleId,"name":value}, data => {
              if(data.retType==0){
                this.setState({
                    validUserno:{
                      retCode:0,
        ...
        if(value == undefined||value == ''){
          ...
          postService(API_PREFIX + `services/system/edition/check/name`,{"id":96371236132,"name":value}, data => {
              if(data.retType==0){
                this.setState({
                    validUserno:{
                      retCode:0,
