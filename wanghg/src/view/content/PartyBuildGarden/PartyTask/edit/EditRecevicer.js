...
export default class EditRecevicer extends Component {
  ...
  dealData = () => {
    ...
    //获取接收人的数据
    getService(API_PREFIX + `services/partybuilding/task/get/getReceivers/${editTaskData.topicId}/${editTaskData.upPartyId}`, data => {
  ...
  onCheck = (checkedKeys, e) => {
    ...
    if (!isChecked) { //取消选中
      if (!isleaf) {//不是叶子节点
        getService(API_PREFIX + `services/partybuilding/task/get/getReceivers/${topicId}/${currentParentKey}`, result => {
