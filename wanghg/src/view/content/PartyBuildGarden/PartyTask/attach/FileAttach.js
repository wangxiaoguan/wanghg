        ...
        result.push(
            {
              ...
              url: API_FILE_VIEW+masterUrl + '/' + temp[0].response.entity[0].filePath,
          ...
          </Button>
          {
            fileArr&&fileArr.map((item,index)=> {
              ...
              return <div key={item.key} className="attach-main">
                 ...
                 {API_CHOOSE_SERVICE===2?<a href={show?API_FILE_VIEW_INNER + show.url:''} style={{display:show&&show.display?'inline-block':'none'}}  target="_blank">下载预览</a>:
                 <a href={show?API_FILE_VIEW + show.url:''} style={{display:show&&show.display?'inline-block':'none'}}  target="_blank">下载预览</a>
                 }
