...
export default class List extends  Component {
  ...
  dealData=()=>{
    ...
    getService(API_PREFIX+'services/partybuilding/partyNews/newsList/get/1/1000',(data)=>{
      if(data.retCode===1){
...
//切换标签
  handleChangeTabs=(activeKey)=>{
    ...
    publishLearn=(record)=>{//可以调用上线的接口
      getService(API_PREFIX+`services/partybuilding/partyNews/newsInfo/publish/${record.id}`,data=>{
        if (data.retCode ===1) {
          Message.success("发布成功!");
          this.props.getData(API_PREFIX+`services/partybuilding/partyNews/newsList/get/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?Q=ispulish_Z_EQ=false`);
        }
        else{
          Message.error(data.retMsg);
        }
 ...
 viewCompletionRate=(record)=>{
  ...
  onTheLine=(record)=>{
    ...
    // let body={ ids:selectedRowKeys }
      console.log(record)
      getService(API_PREFIX+`services/partybuilding/partyNews/newsInfo/onlineState/${record[0]}/1`,data=>{
        if (data.retCode ===1) {
          Message.success("上线成功!");
          this.props.getData(API_PREFIX+`services/partybuilding/partyNews/newsList/get/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?Q=onlineState_S_EQ=0`);
        }
        else{
          Message.error(data.retMsg);
        }
  ...
  offLine=(record)=>{
    ...
    getService(API_PREFIX+`services/partybuilding/partyNews/newsInfo/onlineState/${record[0]}/0`,data=>{
      if (data.retCode ===1) {
        Message.success("下线成功!");
        this.props.getData(API_PREFIX+`services/partybuilding/partyNews/newsList/get/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?Q=onlineState_S_EQ=1`);
      }
      else{
        Message.error(data.retMsg);
      }
