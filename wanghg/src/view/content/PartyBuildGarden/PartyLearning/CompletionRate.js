...
import { Tabs, Divider, Popconfirm, Message, Table, Form, Row, Col, Cascader, Select, Button,Input } from 'antd';
...
export default class CompletionRate extends Component {
  ...
  dealData = () => {
    ...
    postService(API_PREFIX + `services/partybuilding/task/get/getTaskComplete`, body, result => {
  ...
  render() {
    ...
    const columnsDetail = [
      ...
      },
      {
        ...
        key: 'completeStatus',
        render: (data, record) => {
          return record.completeStatus ? '已完成' : '未完成'
        }
      },
    ];
    //rowSelection
