...
export default class Magazine extends Component{
  ...
  componentDidMount(){
    ...
    getService(API_PREFIX + 'services/system/dictionary/magazineSeries/getAllList', data => {
      if (data.retCode == 1) {
        ...
        this.setState({magazineSeriesOption: result,loading: false }, () => {
  ...
  publish = (record) => {
    getService(API_PREFIX + `services/news/magazine/newsInfo/publish/${record.id}/1`, data => {
      if(data.retCode==1){
        message.success('杂志已成功发布！', 2);
        this.setState({
          ispublish:record.ispublish,
        });
        this.props.getData(API_PREFIX+`services/news/magazine/magazineList/get/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
      } else{
        message.error(data.retMsg);
      }
    });
        ...
        render: (text, record,index) => {
          return <span>
            {API_CHOOSE_SERVICE==1?
              <img onClick={this.handleMagazinePreview.bind(this,record.coverimage)} key={index} src={API_FILE_VIEW+record.coverimage} style={{width:'50px',height:'50px'}} />:
