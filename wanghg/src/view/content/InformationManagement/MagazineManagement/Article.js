...
export default class MagazineArticle extends Component{
  ...
  componentDidMount(){
    if(window.sessionStorage.getItem('indexMagazine') == 'one') {
      ...
      getService(API_PREFIX + `services/news/magazine/catalogue/list/${mId}`, data => {
        if (data.retCode == 1) {
          let result = [];
          // result.push({;
          //   key: '', value: '全部',
      ...
      getService(API_PREFIX + `services/news/magazine/catalogue/name/${window.sessionStorage.getItem('contentId')}`, data => {
        if (data.retCode == 1) {
          this.setState({
            catalogueIdOption : [
              {
        ...
        getService(API_PREFIX + `services/system/verify/checkUnique/InformationManagement-Article?${query}`, data => {
          if(data.retCode==0){
            ...
            if (!this.state.isAdd) {
              ...
              postService(API_PREFIX + 'services/news/magazine/article/update', values, data => {
                if (data.retCode == 1) {
                  Message.success('修改成功');
                  this.showArticleList();
                  if (window.sessionStorage.getItem('indexMagazine') == 'two') {
                    this.props.getData(API_PREFIX + `services/news/magazine/article/list/catalogue/${window.sessionStorage.getItem('contentId')}/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
                  } else {
                    this.props.getData(API_PREFIX + `services/news/magazine/article/list/magazine/${window.sessionStorage.getItem('magazineId')}/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
                  }
                }
              });
            } else {
              postService(API_PREFIX + 'services/news/magazine/article/add', values, data => {
  ...
  editArticle = (record) =>{
    ...
    getService(API_PREFIX +`${url}/1/10?Q=id_S_EQ=${record.id}`,data=>{
      if (data.retCode ===1) {
