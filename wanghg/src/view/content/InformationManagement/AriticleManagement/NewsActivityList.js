...
class NewsActivityList extends Component {
  ...
  handleEditModalOK=()=>{
    this.props.form.validateFields((err,values)=>{
      ...
      postService(API_PREFIX + `services/news/artical/specialCategory/setNewsIdSort/update`,body, data => {
        if (data.retCode == 1) {
  ...
  handleAddModalOK=()=>{
    ...
    if(this.state.addType=='文章'){//添加文章
      postService(API_PREFIX + `services/news/artical/specialCategory/addNewsId`,values, data => {
        if (data.retCode == 1) {
  ...
  reflesh=()=>{
    this.props.getData(API_PREFIX+`services/news/artical/specialCategory/getNewsList/${this.state.id}/1/10`);
    this.setState({
      selectedRowKeys:[],
      searchCotent:'',
