...
export default class BirthdayTicketInfo extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pointData: [], //列表展示数据
      ...
      userid: window.sessionStorage.getItem('birthdayTicket-userid') ? JSON.parse(window.sessionStorage.getItem('birthdayTicket-userid')) : '', //userid值 --此页面接口请求的参数
    };
  }

  componentWillUnmount() {
    window.sessionStorage.removeItem('birthdayTicket-userid');
  ...
  componentDidMount() {
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + 'services/system/pointTreasure/changeDetails/specialPoint/list/1/10?Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=2', data => {
  ...
  exportExcel = () => {
    ...
    if (this.state.values === '') {
      query = 'Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=3';
    } else {
      query = this.state.values + '&Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=3';
    }

    let path = API_PREFIX + 'services/system/pointTreasure/export/specialTreasureDetails' + '?' + `${query}`;
    exportExcelService(path, '', '生日券积分变更详情');
  }
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (this.state.queryFilter === '') {
        query = 'Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=3';
      } else {
        query = this.state.queryFilter + '&Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=3';
      }
