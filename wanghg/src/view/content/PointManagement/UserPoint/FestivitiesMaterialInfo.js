...
export default class FestivitiesMaterialInfo extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pointData: [], //列表展示数据
      ...
      userid: window.sessionStorage.getItem('festivitiesMaterial-userid') ? JSON.parse(window.sessionStorage.getItem('festivitiesMaterial-userid')) : '', //userid值 --此页面接口请求的参数
    };
  }

  componentWillUnmount() {
    window.sessionStorage.removeItem('festivitiesMaterial-userid');
  ...
  componentDidMount() {
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + 'services/system/pointTreasure/changeDetails/specialPoint/list/1/10?Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=4', data => {
  ...
  exportExcel = () => {
    ...
    if (this.state.values === '') {
      query = 'Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=5';
    } else {
      query = this.state.values + '&Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=5';
    }

    let path = API_PREFIX + 'services/system/pointTreasure/export/specialTreasureDetails' + '?' + `${query}`;
    exportExcelService(path, '', '过节物资积分变更详情');
  }
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(API_PREFIX + `services/system/pointTreasure/changeDetails/specialPoint/list/${this.state.currentPage}/${this.state.pageSize}?${this.state.values}`, data => {
      if (data.retCode === 1) {
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    getService(API_PREFIX + `services/system/pointTreasure/changeDetails/specialPoint/list/${current}/${pageSize}?${this.state.values}`, data => {
      if (data.retCode === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (this.state.queryFilter === '') {
        query = 'Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=5';
      } else {
        query = this.state.queryFilter + '&Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=5';
      }

      getService(API_PREFIX + `services/system/pointTreasure/changeDetails/specialPoint/list/${this.props.currentPage}/${this.props.pageSize}?${query}`, data => {
        if (data.retCode === 1) {
