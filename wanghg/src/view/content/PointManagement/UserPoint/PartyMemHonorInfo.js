...
export default class PartyMemHonorInfo extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      userid: window.sessionStorage.getItem('partyMemHonor-userid') ? JSON.parse(window.sessionStorage.getItem('partyMemHonor-userid')) : '', //userid值 --此页面接口请求的参数
    };
  }
  ...
  componentDidMount() {
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + 'services/system/pointTreasure/changeDetails/specialPoint/list/1/10?Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=5', data => {
  ...
  exportExcel = () => {
    ...
    if (this.state.values === '') {
      query = 'Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=2';
    } else {
      query = this.state.values + '&Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=2';
    }

    let path = API_PREFIX + 'services/system/pointTreasure/export/specialTreasureDetails' + '?' + `${query}`;
    exportExcelService(path, '', '党员荣誉积分变更详情');
  }
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (this.state.queryFilter === '') {
        query = 'Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=2';
      } else {
        query = this.state.queryFilter + '&Q=userid_I_EQ=' + this.props.userid + '&Q=typeid_I_EQ=2';
      }
