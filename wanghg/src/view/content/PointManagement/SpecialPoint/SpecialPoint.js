...
export default class SpecialPoint extends Component {
  ...
  componentDidMount(){
    ...
    //获取专项积分发放管理列表初始数据
    getService(API_PREFIX + 'services/system/specialTreasureTask/taskList/1/10',data=>{
  ...
  deleteData = ()=>{
    ...
    postService(API_PREFIX+'services/system/specialTreasureTask/delete/task', body, data => {
      if(data.retCode === 1){
  ...
  stopTask=(record)=>{
    ...
    if(record.status==1){status=2}else{status=1}
      getService(API_PREFIX+`services/system/specialTreasureTask/updateJobIdStatus/${record.id}/`+status,data=>{
        if(data.retCode === 1){
          ...
          getService(API_PREFIX + `services/system/specialTreasureTask/taskList/${this.state.currentPage}/${this.state.pageSize}`, data => {
            if(data.retCode === 1){
              this.setState({
                pointData: data.root.list,
                totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) =>{
    getService(API_PREFIX + `services/system/specialTreasureTask/taskList/${current}/${pageSize}`,data =>{
      if(data.retCode === 1){
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
