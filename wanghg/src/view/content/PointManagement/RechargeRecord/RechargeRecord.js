...
export default class RechargeRecord extends Component {
  ...
  componentDidMount() {
    ...
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + 'services/system/userTreasureRecharge/list/1/10', data => {
  ...
  exportExcel = () => {
    let path = API_PREFIX + 'services/system/userTreasureRecharge/export' + '?' + `${this.state.values}`;
  ...
  onPageChange = (current, pageSize) => {
    getService(API_PREFIX + `services/system/userTreasureRecharge/list/${current}/${pageSize}?${this.state.values}`, data => {
      if (data.retCode === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
...
class SearchForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      getService(API_PREFIX + `services/system/userTreasureRecharge/list/${this.props.currentPage}/${this.props.pageSize}?${qFilter}`, data => {
        if (data.retCode === 1) {
