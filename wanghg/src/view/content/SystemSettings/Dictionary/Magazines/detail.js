    ...
    this.props.form.validateFields((err, fieldsValue) => {
      if (!err) {
        ...
        if (this.props.isEdit) {
          ...
          postService(API_PREFIX+'services/system/dictionary/magazineSeries/update',body,data=>{
            if (data.retCode===1) {
              Message.success('修改成功');
              this.props.handleCancel();
              this.props.getData(API_PREFIX+`services/system/dictionary/magazineSeries/getList/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
            }
          });
        }else{
          let body = {
          ...
          postService(API_PREFIX + 'services/system/dictionary/magazineSeries/add', body, data => {
            if (data.retCode === 1) {
