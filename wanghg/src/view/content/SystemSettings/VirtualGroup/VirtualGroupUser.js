...
class VirtualGroupUser extends Component {
  ...
  SetGroupLord = (record) => {
    postService(API_PREFIX + `services/system/virtualGroup/setOwner`,{"groupId":this.state.groupId,"memberId":record.id,"memberName":record.lastName}, data => {
            if (data.retCode == 1) {
              Message.success("设置成功!")
              this.getData(`services/system/virtualGroup/getMemberList/${this.state.groupId}/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
            } else {     
    	...
    	<div className="virtualGroup">
         ...
         <Modal
          ...
          >
            ...
            downlodUrl={API_PREFIX + 'services/system/virtualGroup/template'}/>
          </Modal>
      </div>
    );
  }
