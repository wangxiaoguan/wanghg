...
export default class CarouselDetail extends Component {
  ...
  componentDidMount(){
    getService(API_PREFIX + 'services/lookup/init/relateType', data => {
      ...
      this.setState({
        ...
        if(this.state.isEdit){
          ...
          let url
          if(API_CHOOSE_SERVICE==1){
            url = API_FILE_VIEW + obj.pictureUrl;
          }else{
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(this.state.isEdit){
          ...
          postService(API_PREFIX + 'services/system/loopPicture/update',values, data => {
            if (data.retCode == 1) {
          ...
          values.title = this.state.title;
          values.categoryId = this.state.categoryId;
          postService(API_PREFIX + 'services/system/loopPicture/insert',values, data => {
            console.log('-----------------');
      ...
      <FormItem {...formItemLayout} label="轮播图片" >
        {getFieldDecorator('pictureUrl', {
          ...
          !isCheck ? (
            ...
            <div>
              {API_CHOOSE_SERVICE==1?<Button onClick={() => this.setState({ previewVisible: true, previewImage: API_FILE_VIEW + pictureUrl})}>查看</Button>:
