...
class Content extends Component {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        postService(API_PREFIX + `services/system/homepageContent/update`,{id:this.state.id,showIndex:values.showIndex} ,data => {
            if (data.retCode == 1) {
  ...
  //从首页下线
  offLine = () => {
    getService(API_PREFIX + `services/system/homepageContent/delete/`+this.state.id,data => {
            if (data.retCode == 1) {
