...
export default class Detail extends Component {
  ...
  getData = () => {
    getService(API_PREFIX + `services/activity/orderActivity/list/1/10?Q=id_S_EQ=${this.state.id}`, data => {
      if (data.retCode === 1) {
  ...
  render() {
    return <EventAndInfoAdd key={this.state.update} type="event" steps={this.state.steps} style="detail" id={{ id: this.state.id }} belonged="order" dpRootId={this.state.dpRootId}/>;
  }
}
const eventForm = [
