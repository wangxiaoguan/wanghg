...
export default class SetShopping extends Component {
  ...
  onPageChange = (currentPage) => {
    this.setState({ 
      ...
      getService(API_PREFIX + `services/activity/orderActivity/goods/list/${this.state.currentPage}/10?Q=status_I_EQ=1&Q=name_S_LK=`+this.state.value, data => {
  ...
  showModal = () => {
    getService(API_PREFIX + `services/activity/orderActivity/goods/list/1/10?Q=status_I_EQ=1`, data => {
      data.root.list.map((item,index) => {
        item.key = index
      })
       this.setState({
  ...
  addShopping = () => {
    ...
    postService(API_PREFIX+'services/activity/orderActivity/goods/add/'+this.state.activityId,{"ids":values},data=>{
      if(data.retCode == 1 ){
      	this.handleCancel()
      }
    });
  ...
  onSearch = (value) => {
    getService(API_PREFIX + `services/activity/orderActivity/goods/list/1/10?Q=status_I_EQ=1&Q=name_S_LK=`+value, data => {
      data.root.list.map((item,index) => {
        item.key = index
      })
       this.setState({
