...
export default class Add extends Component {
  ...
  getData=()=>{
    getService(API_PREFIX +`services/activity/questionnaireActivity/list/1/10?Q=id_S_EQ=${this.state.id}`,data=>{
      if (data.retCode ===1) {
  ...
  render() {
    return <EventAndInfoAdd key={this.state.update} type="event" steps={this.state.steps} style="edit" id={{ id: this.state.id }} timePush={timePush} save={'/EventManagement/Questionnaire/List'}belonged="questionnaire" dpRootId={this.state.dpRootId}/>;
  }
}
