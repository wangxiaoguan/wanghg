...
export default class Detail extends Component {
  ...
  getData = () => {
    getService(API_PREFIX + `services/activity/examActivity/list/1/10?Q=id_S_EQ=${this.state.id}`, data => {
      if (data.retCode === 1) {
  ...
  render() {
    return <EventAndInfoAdd key={this.state.update} type="event" steps={this.state.steps} style="detail" id={{ id: this.state.id }} save={'/EventManagement/Examination/List'} belonged="examination" dpRootId={this.state.dpRootId}/>;
  }
}
const radioOption = [
  { label: '是', value: true },
