...
//普通评论转-精彩评论-普通评论
  handleChange=(e,record)=>{
    ...
    postService(API_PREFIX+'services/activity/comment/update',body,data=>{
      console.log("普通评论/精彩评论",data);
      if(data.retCode==1){
        message.success('操作成功');
        // history.go()
        this.props.getData(API_PREFIX+`services/activity/comment/list/${this.state.targetType}/1/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
      }
    });
  }
  //排序发生改变
  handleOrderChange=(e)=>{
    ...
    //重新获取数据
    this.props.getData(API_PREFIX+`services/activity/comment/list/${this.state.targetType}/${this.state.order}/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
  }
  render() {
