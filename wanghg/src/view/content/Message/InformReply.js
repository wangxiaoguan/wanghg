...
class InformReply extends Component {
  ...
  requestData() {
    if (this.props.id) {
      getService(API_PREFIX + `services/message/report/reportInfo/get/${this.props.id}`, (res) => {
  ...
  requestReply() {
    if (this.props.id) {
      postService(API_PREFIX + `services/message/report/updateDispose/${this.state.reply}/${this.props.id}`, null, (res) => {
        this.afterDeal(res);
      });
    }
  }
  ...
  requestIgnore() {
    if (this.props.id) {
      postService(API_PREFIX + `services/message/report/updateNeglect/${this.state.reply}/${this.props.id}`, null, (res) => {
        this.afterDeal(res);
      });
    }
  }
          ...
          <FormItem className="itemImage" label="举报图片" {...FormItemLayout}>
            {
              ...
              imgUrl.map((item) => {
                return <span>
                  {API_CHOOSE_SERVICE==1?<Zmage src={API_FILE_VIEW + item} alt="" />:<Zmage src={API_FILE_VIEW_INNER + item} alt="" />
