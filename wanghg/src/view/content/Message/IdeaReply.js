...
class IdeaReply extends Component {
  ...
  requestReply() {
    ...
    postService(API_PREFIX + `services/message/feedback/update/${this.props.id}/${this.state.replyContent}`,
      {
        id: this.props.id,
          ...
          <FormItem className="formItemImage" label="图片" {...FormItemLayout}>
            {
              ...
              images.map((item, index) => {
                return <span>
                  {API_CHOOSE_SERVICE==1?<Zmage key={index} src={API_FILE_VIEW + item} alt='' />:
