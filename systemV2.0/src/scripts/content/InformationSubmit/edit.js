            ...
            if(res.flag){
                ...
                this.setState({spin:false,list1,list2,list3,list4,list5,list6,tableList3,tableList4,tableList5,tableList6,allData:{...data,month:new Date().getMonth()+1,}},()=>{
                    this.props.form.setFieldsValue({
                        ...
                        govOrderStartTime:data.govOrderStartTime?moment(data.govOrderStartTime):null,
                        govOrderEndTime:data.govOrderEndTime?moment(data.govOrderEndTime):null,

                        systemAdapteStartTime:data.systemAdapteStartTime?moment(data.systemAdapteStartTime):null,
                        systemAdapteEndTime:data.systemAdapteEndTime?moment(data.systemAdapteEndTime):null,

                        systemTestStartTime:data.systemTestStartTime?moment(data.systemTestStartTime):null,
                        systemTestEndTime:data.systemTestEndTime?moment(data.systemTestEndTime):null,

                        systemTestRunStartTime:data.systemTestRunStartTime?moment(data.systemTestRunStartTime):null,
                        systemTestRunEndTime:data.systemTestRunEndTime?moment(data.systemTestRunEndTime):null,

                        assessmentStartTime:data.assessmentStartTime?moment(data.assessmentStartTime):null,
                        assessmentEndTime:data.assessmentEndTime?moment(data.assessmentEndTime):null,

                        systemOnlineStartTime:data.systemOnlineStartTime?moment(data.systemOnlineStartTime):null,
                        systemOnlineEndTime:data.systemOnlineEndTime?moment(data.systemOnlineEndTime):null,

                        projectFinishStartTime:data.projectFinishStartTime?moment(data.projectFinishStartTime):null,
                        projectFinishEndTime:data.projectFinishEndTime?moment(data.projectFinishEndTime):null,

                        financialAccountsStartTime:data.financialAccountsStartTime?moment(data.financialAccountsStartTime):null,
                        financialAccountsEndTime:data.financialAccountsEndTime?moment(data.financialAccountsEndTime):null,
    
                        curInvestmentPlaceTime:data.curInvestmentPlaceTime?moment(data.curInvestmentPlaceTime):null,
                        ...
                        curInvestmentPlaceMoney:String(data.curInvestmentPlaceMoney),
                        curInvestmentPlanMoney:String(data.curInvestmentPlanMoney),
                        curInvestmentPayMoney:String(data.curInvestmentPayMoney),

                        //六、备注
                        addInvestmentMoney:data.addInvestmentMoney?String(data.addInvestmentMoney):'',
                        addTerminalsNum:data.addTerminalsNum?String(data.addTerminalsNum):'',
                        addServersNum:data.addServersNum?String(data.addServersNum):'',
                        ebInvestmentMoney:data.ebInvestmentMoney?String(data.ebInvestmentMoney):'',
                        ebTerminalsNum:data.ebTerminalsNum?String(data.ebTerminalsNum):'',
                        ebServersNum:data.ebServersNum?String(data.ebServersNum):'',
                        otherItem:data.otherItem,

                        //七、问题与建议
                        isSmSuggestion:data.isSmSuggestion,
                        notSmSuggestion:data.notSmSuggestion,
                        contactName:data.contactName,
                        officePhone:data.officePhone,
                        contactPhone:data.contactPhone,

                        //统计周期
                        curReportStartTime:data.curReportStartTime?moment(data.curReportStartTime):null,
                        curReportEndTime:data.curReportEndTime?moment(data.curReportEndTime):null,
                        appReportStartTime:data.appReportStartTime?moment(data.appReportStartTime):null,
                        appReportEndTime:data.appReportEndTime?moment(data.appReportEndTime):null,
                        devReportStartTime:data.devReportStartTime?moment(data.devReportStartTime):null,
                        devReportEndTime:data.devReportEndTime?moment(data.devReportEndTime):null,
