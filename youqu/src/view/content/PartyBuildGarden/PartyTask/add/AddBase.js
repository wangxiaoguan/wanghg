...
import { BEGIN,setFormData,setTopicId,setPartyId } from '../../../../../redux-root/action';
...
@connect(
    state => ({
      ...
      selectRowsData: state.selectRowsData,  //获取选中行的数据
      getArticleData:state.articleData,//获取添加文章的数据
      getActivityData:state.activityData,//获取添加活动的数据
      getFileData :state.fileData,//获取添加文件的数据
...
export default class AddBase extends  Component {
    ...
    if(flowData&&Object.keys(flowData).length!=0){
      ...
      if(flowData.topicId){//不为空
        ...
        this.setState({partyOptions});
          getService(ServiceApi+`services/partybuilding/task/get/selectTaskTypeByTopicId/${flowData.topicId}`,(data)=>{
          if(data.retCode===1){
        ...
        //任务主题    根据选中的任务类型，获取对应任务类型对应的 是否线下会议记录
          getService(ServiceApi+`services/partybuilding/task/taskTopicList/get/1/10?Q=id_I_EQ=${flowData.topicId}`,(data)=>{
          if(data.retCode===1){
  ...
  handleOk=()=>{
    ...
    getService(ServiceApi+`services/partybuilding/task/get/selectTopicIdByUserLevel/${selectedData[0].level}`,(data)=>{
      if(data.retCode===1){
        topicOption = [];
        data.root.list&&data.root.list.map(item=>{
    ...
    //关联上级工作部署
    getService(ServiceApi+`services/partybuilding/task/get/queryUntiedTask/${selectedData[0].partyId}`,data=>{
      if(data.retCode===1){
        //通过接口获取多选框中的数据
        let checkBoxOption=[];
        data.root.list&&data.root.list.map(item=>{
  ...
  handleSubmit=(e,next)=>{   //如果next不为空，则为“保存并下一步”
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if(this.props.flowData&&this.props.flowData.id){//编辑
        ...
        //新增的接口
        postService(ServiceApi+'services/partybuilding/task/add/insertTask',values,data=>{
          if(data.retCode===1){ //增加成功  返回一系列数据，放入redux中
  ...
  timePush=(id,add)=>{
    ...
    let  body= {
      ...
      operateType: 1,
      queryType: 'post',
      queryUrl: ServiceApi+'services/partybuilding/task/update/updateTasksOnline',
      queryValue: {
        ids: [id]
      },
    ...
    postService(ServiceApi + `services/automation/job/${add?'add':'update'}`, body, timeData => {
      if (timeData.retCode === 1) {
        let jobId = timeData.root.id;
        getService(ServiceApi + `services/partybuilding/task/insert/addJobId/${id}/${jobId}`, bindData => {
  ...
  handleSearch=(data,url)=>{
    ...
    this.props.setTableData(ServiceApi + `${url}/1/10?Q=userName_S_LK=${data.target.value}`);
  }
  //任务主题选择后
  handleSelect=(value)=>{
    ...
    getService(ServiceApi+`services/partybuilding/task/get/selectTaskTypeByTopicId/${value}`,(data)=>{
      if(data.retCode===1){
        typeOption = [];
        data.root.list&&data.root.list.map(item=>{
          typeOption.push({key: item.id, value: item.name});
    ...
    //任务主题    根据选中的任务类型，获取对应任务类型对应的 是否线下会议记录
    getService(ServiceApi+`services/partybuilding/task/taskTopicList/get/1/10?Q=id_I_EQ=${value}`,(data)=>{
      if(data.retCode===1){
        if(data.root.list.length>0){
           let info=data.root.list[0];
