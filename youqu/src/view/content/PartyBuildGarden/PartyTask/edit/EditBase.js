...
@connect(
    state => ({
      ...
      editorData: state.editorData, //获取富文本中的数据
...
export default class EditBase extends  Component {
  ...
  dealData=()=> {
    ...
    if (this.props.editTaskData !== this.state.editTaskData) {
      this.setState({ editTaskData: this.props.editTaskData },()=>{
        ...
        //关联上级工作部署
        getService(ServiceApi+`services/partybuilding/task/get/queryUntiedTask/${editTaskData.upPartyId}`,data=>{
          if(data.retCode===1){
            //通过接口获取多选框中的数据
        ...
        if(editTaskData){
          ...
          if(editTaskData.topicId){ //不为空
            ...
            getService(ServiceApi+`services/partybuilding/task/taskTopicList/get/1/10?Q=id_I_EQ=${editTaskData.topicId}`,(data)=>{
              if(data.retCode===1){
  ...
  handleSubmit=(e)=>{   //如果next不为空，则为“保存并下一步”
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if(values.isTimed==1){//是  定时发布
        ...
        postService(ServiceApi + `services/automation/job/${this.props.editTaskData.jobId?'update':'add'}`, body, timeData => {
          if (timeData.retCode === 1) {
            let jobId = timeData.root.id;
            getService(ServiceApi + `services/partybuilding/task/insert/addJobId/${this.props.editTaskData.id}/${jobId}`, bindData => {
  ...
  handleSearch=(data,url)=>{
    ...
    // this.props.setTableData(ServiceApi + `${url}/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?Q=userName_S_LK=${value}`);
  }
  //任务主题选择后
  handleSelect=(value)=>{
    ...
    //任务类型
    let typeOption =this.state.typeOption; //
    getService(ServiceApi+`services/partybuilding/task/taskTypeList/get/1/1000?Q=topicId_I_EQ=${value}`,(data)=>{
      if(data.retCode===1){
        data.root.list&&data.root.list.map(item=>{
          typeOption.push({key: item.id, value: item.name});
        });
