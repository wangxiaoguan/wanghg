  ...
  handleSubmit=(e)=>{
    ...
    this.props.form.validateFields((err,fieldsValue)=>{
      ...
      if(this.state.flag=='point'){//调用经验值操作的接口
        postService(ServiceApi+'services/system/systemAndCompanyUser/updatePoint',values,data=>{
          if(data.retCode==1){
            message.success("操作成功");
             if(this.state.whichPage=='user'){//用户管理页面
               ...
               console.log("游客管理页面");
               this.props.getData(ServiceApi+ `services/system/touristUser/list/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
             }
            this.props.ok();
          }else{
        ...
        postService(ServiceApi+'services/system/systemAndCompanyUser/updateTreasure',values,data=>{
          if(data.retCode==1){
            message.success("操作成功");
            if(this.state.whichPage=='user'){//用户管理页面
              console.log("用户管理页面");
