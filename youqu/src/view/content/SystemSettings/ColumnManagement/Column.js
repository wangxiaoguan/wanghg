...
@connect(state => ({
  ...
  selectDetail: state.treeSelectData.selectDetail,
  selectTreeData: state.treeSelectData.DynamicTree,
  ...
  componentDidUpdate(){
    ...
    if (selectChange && selectTreeData&&selectTreeData.length>0) {
      ...
      if(selectTreeData.toString().indexOf('unsavetreenodekey')<0){  //不包含模拟的id
        getService(ServiceApi + `services/system/cateogry/categoryAuthInfo/get/${selectTreeData}`,data=>{
