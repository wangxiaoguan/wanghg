...
import {setCheckTreeData,setSelectTreeData} from'../../../../../redux-root/action';
...
export default class TagDetail extends Component {
  ...
  getTreeData = () => {
    ...
    getService(
      ServiceApi + 'services/system/cateogry/categoryList/get',
      data => {
        if (data.retCode === 1) {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if(this.state.isEdit){
          ...
          values.id = obj.id;
          postService(ServiceApi + 'services/system/homepageTag/update',values, data => {
            if (data.retCode == 1) {
          ...
          postService(ServiceApi + 'services/system/homepageTag/insert',values, data => {
  ...
  handleValidName= (rule, value, callback) => {
    if(value == ''||value==undefined){
      ...
      getService(ServiceApi + `services/system/verify/checkUnique/HomeLayout-Tag?${query}`, data => {
        if(data.retCode==0){
