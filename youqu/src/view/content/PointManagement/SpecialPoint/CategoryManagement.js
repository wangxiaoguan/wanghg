...
export default class CategoryManagement extends Component {
  ...
  componentDidMount() {
    //获取类别管理列表初始数据
    getService(ServiceApi + 'services/system/specialTreasureTask/typeList/1/10', data => {
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(ServiceApi + `services/system/specialTreasureTask/typeList/${this.state.currentPage}/${this.state.pageSize}`, data => {
      if (data.retCode === 1) {
        this.setState({
          categoryData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    getService(ServiceApi + `services/system/specialTreasureTask/typeList/${current}/${pageSize}`, data => {
      if (data.retCode === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          categoryData: data.root.list,
...
class CategoryForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      postService(ServiceApi + 'services/system/specialTreasureTask/update/type', values, data => {
        if (data.retCode === 1) {
