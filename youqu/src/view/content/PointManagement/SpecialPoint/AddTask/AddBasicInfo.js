...
export default class AddBasicInfo extends Component{
  ...
  timePush=(id,add,deliverydate)=>{
    let  body= {
      ...
      operateType: 1,
      queryType: 'post',
      queryUrl: ServiceApi+'services/system/specialTreasureTask/putUserSpecialPoint',
      queryValue: {
        ids: [id]
      },
    ...
    postService(ServiceApi + `services/automation/job/${add?'add':'update'}`, body, timeData => {
      if (timeData.retCode === 1) {
        ...
        getService(ServiceApi + `services/system/specialTreasureTask/updateJobId/${id}/${jobId}`, bindData => {
