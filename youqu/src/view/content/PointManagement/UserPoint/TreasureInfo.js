...
export default class TreasureInfo extends Component {
  ...
  componentDidMount() {
    //获取用户积分管理列表初始数据
    getService(ServiceApi + `services/system/pointTreasure/changeDetails/treasure/list/${this.state.userid}/1/10`, data => {
  ...
  exportExcel = () => {
    let path = ServiceApi + `services/system/pointTreasure/export/treasureDetails/${this.state.userid}` + '?' + `${this.state.values}`;
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(ServiceApi + `services/system/pointTreasure/changeDetails/treasure/list/${this.state.userid}/${this.state.currentPage}/${this.state.pageSize}?${this.state.values}`, data => {
      if (data.retCode === 1) {
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    getService(ServiceApi + `services/system/pointTreasure/changeDetails/treasure/list/${this.state.userid}/${current}/${pageSize}?${this.state.values}`, data => {
      if (data.retCode === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
...
class SearchForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      getService(ServiceApi + `services/system/pointTreasure/changeDetails/treasure/list/${this.props.userid}/${this.props.currentPage}/${this.props.pageSize}?${this.state.queryFilter}`, data => {
        if (data.retCode === 1) {
