...
export default class UserPoint extends Component {
  ...
  componentDidMount(){
    ...
    //获取用户积分管理列表初始数据
    getService(ServiceApi + 'services/system/pointTreasure/list/1/10',data=>{
    ...
    //部门下拉框数据
    getService(ServiceApi + 'services/system/organization/getFirstGradeOrg',data=>{
  ...
  exportExcel = () =>{
    ...
    let path = ServiceApi + 'services/system/pointTreasure/export/pointTreasure' + '?' + `${queryFilter}`;
  ...
  refreshData = () => {
    ...
    getService(ServiceApi + `services/system/pointTreasure/list/${this.state.currentPage}/${this.state.pageSize}?${queryFilter}`, data => {
      if(data.retCode === 1){
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) =>{
    ...
    getService(ServiceApi + `services/system/pointTreasure/list/${current}/${pageSize}?${queryFilter}`,data =>{
      if(data.retCode === 1){
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
  ...
  render() {
    ...
    return <Spin spinning={this.state.loading}><div className="custom-table">
      ...
      <Modal
        ...
        destroyOnClose={true}
      >
        <ImportPart importUrl="/services/system/import/treasure" downlodUrl={ServiceApi + 'services/system/pointTreasure/template/export/pointTreasure'}
...
class SearchForm extends Component{
  ...
  handleSubmit= e => {
    ...
    this.props.form.validateFields((err, fieldsValue) =>{
      ...
      getService(ServiceApi + `services/system/pointTreasure/list/${this.props.currentPage}/${this.props.pageSize}?${queryFilter}`,data => {
        if(data.retCode === 1){
