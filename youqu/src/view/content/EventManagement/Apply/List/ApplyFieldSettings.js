...
export default class ApplyFields extends Component {
  ...
  componentDidMount(){
    //获取报名字段的数据
    getService(ServiceApi + 'services/system/dictionary/enrollField/getList/1/10000', data => {
      if (data.retCode == 1) {
  ...
  handleSubmit=(e)=>{
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      if (!err) {
        ...
        postService(ServiceApi + 'services/activity/signUpActivity/updateRegistryForm',fieldsValue, data => {
          if (data.retCode == 1) {
            console.log('++++++++++++++');
