...
import { setSelectTreeData, setCheckTreeData, setChangeData} from '../../../redux-root/action';
...
@connect(
  state => ({
    selectTreeData: state.treeSelectData,
    isCheck: state.treeCheckData.isCheck,
  }),
  dispatch => ({
...
export default class TreeList extends Component {
  ...
  delete = async() => {
    ...
    const searchData = (selectKey, tree) => {
      ...
      postService(ServiceApi + this.props.deleteUrl +'/'+ selectKey, '',data =>
      {
        if (data.retCode===1) {
         console.log('删除成功')
         for (let index = 0; index < tree.length; index++) {
