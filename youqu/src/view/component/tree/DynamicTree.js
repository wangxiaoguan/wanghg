...
import { setLoading} from '../../../redux-root/action';
...
@connect(
  state => ({
    ...
    getChangeData: state.treeChangeData,
  }),
  dispatch => ({
  ...
  handleSubmit = () => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (this.state.queue.startQueue) {
        ...
        start.push(
          new Promise((resolve, reject) => {
            postService(ServiceApi + `${this.props.startUrl}`, body, data =>
            {
              if (data.retCode===1) {
                resolve(data);
              }else{
      ...
      if (this.state.queue.stopQueue) {
        ...
        stop.push(
          new Promise((resolve, reject) => {
            postService(ServiceApi + this.props.stopUrl, body, data =>
            {
              if (data.retCode===1) {
                resolve(data);
              }else{
      ...
      if (this.state.queue.addQueue) {    //平级
        this.state.queue.addQueue.map((item, index) => {
          ...
          add.push(
            new Promise((resolve, reject) => {
              postService(ServiceApi + `${this.props.addUrl}`, body, data =>
              {
                if (data.retCode===1) {
                  resolve(data);
                }else{
      ...
      if (this.state.queue.deleteQueue) {
        this.state.queue.deleteQueue.map(item => {
          deletePro.push(
            new Promise((resolve, reject) => {
              postService(ServiceApi + this.props.deleteUrl +'/'+ item, '',data =>
              {
                if (data.retCode===1) {
                  resolve(data);
                }else{
      ...
      if (this.state.queue.updateQueue) {
        this.state.queue.updateQueue.map(item => {
          ...
          update.push(
            new Promise((resolve, reject) => {
              postService(ServiceApi + `${this.props.updateUrl}`, body, data =>
              {
                if (data.retCode===1) {
                  resolve(data);
                }else{
