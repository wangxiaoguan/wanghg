...
import { getService } from '../view/content/myFetch.js';
import API_PREFIX from '../view/content/apiprefix.js';
import { getLen} from '../utils/ruleConfig';
const pattern = new RegExp('[`~!%#$^&*()=|{}\':;\',\\[\\]<>/?~！#￥……&*（）——|{}【】‘；：”“\'。，、？]'); 
// const videoRegUrl=  /(http|https)\:\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/gi;
const videoRegUrl=/(http|ftp|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-\@?^=%&/~\+#])?/;//xwx/2018/12/11  验证网址正则
...
export const checkVideo = (rule, value, callback) => {
  if (!value) {
    callback('网页地址为必填项');
  } else if(videoRegUrl.exec(value)){
    callback();
  }else{
    callback('网页地址不对（地址为网页地址http://或https://格式）');
  }
};
export const checkAddress = (rule, value, callback) => {
  if(videoRegUrl.exec(value)||!value){
    callback();
  }else{
    callback('地址不对（地址格式为http://或https://）');
  }
};
...
export const illegalInput = (rule, value, callback) => {
...
export const checkInput = (rule, value, callback) => {
  if (pattern.test(value)) {
    callback('请勿输入非法字符');
  } else if(value&&value.length>16){//(党建园地=》新建党建任务)修复新建直接点击保存或保存并下一步报错的问题xwx2019/2/26
    callback('任务名称最大长度不能超过16');
  }else if(!value){
    callback('任务名称为必填项不能为空');
  }else{
    callback()
  }
};
export const checkPointNumber = (rule, value, callback,name) => {
    if(value<0){
        callback('请勿输入负数');
    }
    // else if(value>9999){
...
export const checkTopic = (rule, value, callback) => {
  if (pattern.test(value)) {
    callback('请勿输入非法字符');
  } else if(value.length>16){
    callback('栏目名称最大长度不能超过16');
  }else if(!value){
    callback('栏目名称为必填项不能为空');
  }else{
    callback()
  }
};
export const specialCharacter = (rule, value, callback, massage, uniqueness, id, optionValue,validateLen) => {
  let pattern = new RegExp('[`~!@#$^&*()=|{}\':;\',\\[\\].<>/?~！@#￥……&*（）——|{}【】‘；：”“\'。，、？]');
  if (value) {      //非空
    if (!pattern.test(value)) {     //不是非法字符
      if (validateLen) {
        if (getLen(value) > validateLen.length) {
          callback([new Error(validateLen.errorField)]);
        } else {
          if (uniqueness) {             //唯一性验证
            if (optionValue) {            //选项验证
              let arr = [...optionValue.arrays];
              arr.splice(optionValue.index, 1);
              if (arr.some(item => item.fieldName === value)) {
                callback([new Error('存在重复的选项!')]);
              } else {
                callback();
              }
            } else {
              ...
              // getService(
              //   API_PREFIX +
              //   `services/system/verify/checkUnique/SystemSettings-Dictionary?Q=fieldName_S_EQ=${value}&Q=groupCode_S_EQ=${uniqueness}${id ? `&Q=parentId_S_EQ =${id}` : ''}`,
              //   data => {
              //     if (data.retCode === 1) {
              //       callback();
              //     } else {
              //       callback([new Error('存在重复的名称!')]);
              //     }
              //   }
              // );
            }
