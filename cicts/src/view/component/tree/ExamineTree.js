...
export default class ExamineTree extends Component {
  constructor(props) {
    super(props);
    this.dataList = [];
    this.state = {
    ...
    if(columnPower){
        ...
        getService(API_PREFIX + 'services/web/system/tree/category/getList?Q=categoryState=1', data => {
            if (data.status === 1) {
                ...
                this.setState({treeData,treeDataKey: this.state.treeDataKey + 1,checkedKeys: this.props.checkedKeys});
            }
        })
    }
  }
    ...
    return (
      <div className="selectable-tree">
        ...
        <Tree
          ...
          checkedKeys={(this.props.checkable && treeData && treeData.length > 0) ? this.state.checkedKeys : []}
