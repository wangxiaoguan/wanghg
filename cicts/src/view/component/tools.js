...
export const cloneObj=(obj)=>{
  let str='',newobj = obj && obj.constructor === Array ? [] : {};
...
export const getBreadcrumb=(menu,str)=>{
  if(str){
        ...
        if(v.url===str){
          ...
          if(v.subMenu&&v.subMenu.length>0){
            v.subMenu.map((item) => {
              ...
              if(item.subBreadcrumb){
                item.subBreadcrumb.map((item) => {
                  if(item.url == str){
                    let d={
                      title:v.title,
                      url:v.url,
          ...
          }else if (v.subBreadcrumb && v.subBreadcrumb.length > 0) {
          
            v.subBreadcrumb.map((item) => {
              if(item.url == str){
                let d = {
                  title: v.title,
                  url: v.url,
   ...
   /* const getTitles=(menu,str)=>{
      menu.map((item,index)=>{
        let d = { title: item.title, url: item.url, unClickable: item.unClickable };
        tmp.push(d);
        if (item.url === str) {
          // let d = {
          //   title: item.title,
          ...
          if (item.subMenu.filter(_ => _.url === str).length>0) {
            getTitles(item.subMenu,str);
          }else{
            getTitles(item.subMenu, str);
          }
        }else if(item.subBreadcrumb){
          if (item.subBreadcrumb.filter(_ => _.url === str).length>0) {
            // let d = { title: item.title, url: item.url, unClickable: item.unClickable };
            // tmp.push(d);
            getTitles(item.subBreadcrumb, str);
          }else{
            getTitles(item.subBreadcrumb, str);
          }
        }else{
