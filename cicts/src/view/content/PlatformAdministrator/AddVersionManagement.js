...
class AddVersionManagement extends Component {
  ...
  componentWillMount(){
     if(this.state.flag == 'true'){
      ...
      getService(API_PREFIX + `services/web/system/edition/getEdition/`+ this.state.roleId,data => {
        if (data.status == 1) {
            const arr5 = [];
            const arr6= [];
            data.root.object.codeList.length!==0&&data.root.object.codeList.map((item) => {
              if(item.length == 11){
                arr5.push(item);
              }else{
                arr6.push(item);
          ...
          this.setState({
            value:data.root.object.codeList,
...
/*    let param = GetQueryString(location.hash,['flag','roleId','isEdit']);
    ...
    getService(API_PREFIX + `services/web/system/edition/permission/menu/all`,data => {
      if (data.status == 1) {
        console.log('menuPermissionsData==>',data);
        this.setState({
          menuPermissionsData:data.root.object,
        });
      }else{     
        message.error(data.errorMsg);
      }
      ...
      if (!err) {
          ...
          if(data != undefined){
             if(this.state.isEdit){
                ...
                postService(API_PREFIX + `services/web/system/edition/update`,data,data => {
                  if (data.status == 1) {
                    this.setState({
                      operation:[],
                      sum:[],
                ...
                postService(API_PREFIX + `services/web/system/edition/add`,data,data => {
                  if (data.status == 1) {
                    this.setState({
                      operation:[],
                      sum:[],
 ...
 /* handleSubmit(){
  ...
  onTabClick = (key) =>{
    ...
    this.setState({
      ...
      if(key == '2'){
        postService(API_PREFIX + `services/web/system/edition/permission/operation`,{menus:this.state.operationPermissions},data => {
          if (data.status == 1) {
            this.setState({
  ...
  handleConfirmPassword = (rule, value, callback) => {
      ...
      if(this.state.isEdit){
        if(value == undefined||value == ''){
          ...
          postService(API_PREFIX + `services/web/system/edition/checkName`,{"id":this.state.roleId,"name":value}, data => {
              if(data.status==1){
                this.setState({
                    validUserno:{
                      retCode:0,
        ...
        if(value == undefined||value == ''){
          ...
          postService(API_PREFIX + `services/web/system/edition/checkName`,{"name":value}, data => {
              if(data.status==1){
                this.setState({
                    validUserno:{
                      retCode:0,
        ...
        <TabPane tab="菜单权限" key="1">
          {
            this.state.menuPermissionsData&&this.state.menuPermissionsData.map((item,index) => {
             ...
             <CheckboxGroup options={item.options} disabled={disabled}  value={this.state.menu} onChange={this.onChange.bind(this, item.value)} /></div>;
            })
          }
        </TabPane>
            ...
            this.state.operationPermission&&this.state.operationPermission.map((item1,index1) => {
              ...
              { 
                item1.options.map((item,index) => {
                  return <div>
                    {
                      // item.label==='权重管理'?null:
                      <div key={index} className="div" style={{height:"50px",display: "table"}}> 
