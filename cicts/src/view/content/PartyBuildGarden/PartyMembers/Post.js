...
class Post extends Component {
  ...
  addPost = (e) => {
    ...
    if (postArray.length > 4) {
      ...
      if(postArray.length && postArray[postArray.length - 1].id) {
        idlength = postArray[postArray.length - 1].id + 1
      }
  ...
  deletePost = (key) => {
    ...
    let initPost = this.props.post && this.props.post.indexOf(';') != -1 ? this.props.post.split(';') : [this.props.post];
    let list = initPost.filter(item => {
      return item && item.indexOf(postData[key] && postData[key].value) != -1
    })
    console.log(list)
    ...
    postList.push(list)
    let postStr = postList.length < 2 ? list[0] : postList.join(';')
    this.props.postDelelte(postStr)
