...
export default class Note extends  Component {
  ...
  constructor(props){
    ...
    this.state={
      ...
      activeKey: GetQueryString(location.hash, ['activeKey']).activeKey || '1',
      fileModal:false,
      currentPage:1,
      pageSize:10,
      dataSource:[
        // {name:'文件1',id:'1',url:'http://ctyoss.chutianyun.gov.cn:8060/yq_fiberhome_test/3/13/10/8/news/20191017030004415144.pdf'},
        // {name:'文件2',id:'2',url:'http://ctyoss.chutianyun.gov.cn:8060/yq_fiberhome_test/3/13/10/8/news/20191017030004415643.doc'},
        // {name:'文件2',id:'3',url:'http://ctyoss.chutianyun.gov.cn:8060/yq_fiberhome_test/3/13/10/8/news/20191014090206324741.jpeg'},
        // {name:'文件4',id:'4',url:'http://ctyoss.chutianyun.gov.cn:8060/yq_fiberhome_test/3/13/10/8/news/20191017030004415144.pdf'}
      ],
      selectedRowKeys:[],
      totalNum:12,
      downUrl: '', //不同环境下载地址的前缀
  ...
  showFile = (record) => {
      this.setState({fileModal:true,selectedRowKeys: [], dataSource: record.noteAttaches ? record.noteAttaches : []})
      ...
      <Spin spinning={this.state.loading}>
          <div>
              <TableAndSearch
                  ...
                  columns={columns}
                  search={search}
                  url={'services/web/news/learningNotes/getList'}
                  deleteBtn={hasDeleteBtn?{order:2,url:'services/web/news/learningNotes/delete'}:null}
                  urlfilter={`Q=newsId=${this.state.noteId}`}
                  goBackBtn={{order: 2, label: '返回', url: `/PartyBuildGarden/PartyLearning?id=${this.state.activeKey}`}}
              />
              <Modal
                width={600}
                title=""
