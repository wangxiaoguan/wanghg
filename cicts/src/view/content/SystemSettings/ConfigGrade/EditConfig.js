...
export default class AddConfig extends  Component {
  ...
  dealData=()=>{
    //根据传入的id，获取党建任务的详情信息
    getService(API_PREFIX+`services/web/configInfo/getList/1/10?Q=id=${this.state.id}`,data=>{
      if(data.status===1){
        if(data.root.list){
  ...
  handleSubmit=(e)=>{   //如果next不为空，则为“保存并下一步”
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      postService(API_PREFIX+'services/web/configInfo/update',values,data=>{
        ...
        <div>
              <Form onSubmit={this.handleSubmit}>
                      ...
                      rules: [
                        {
                          required:true ,
                          whitespace: true,
                          message: '配置名称为必填项且最大长度为60',
                          max:60,
                        },
                      ],initialValue:editTaskData?editTaskData.name:'',
                    })
                    (<Input  />)
                  }
                </FormItem>
                <FormItem {...formItemLayout}  label="配置值">
                  {
                    getFieldDecorator('value',{
                      ...
                      ],initialValue:editTaskData?editTaskData.value:'',
                    })
                    (<Input />)
                  }
                </FormItem>
                <FormItem {...formItemLayout}  label="启用">
                  {
                    getFieldDecorator('isEnable',{
                      ...
                      ],initialValue:editTaskData?editTaskData.isEnable:'',
                    })
                    (
                      <RadioGroup>
                ...
                <FormItem {...formItemLayout} label="配置描述">
                  {
                    getFieldDecorator('description',{
                      ...
                      ],initialValue:editTaskData?editTaskData.description:'',
                    })
                    (
                ...
                <Row>
                  ...
                  <Col span={3}><Button className="queryBtn"
