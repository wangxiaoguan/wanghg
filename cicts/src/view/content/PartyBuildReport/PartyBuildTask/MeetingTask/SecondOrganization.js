...
export default class SecondOrganizationFromMeet extends Component{
  constructor(props){
    super(props);
    this.state = {
      data:{
  ...
  render(){
    ...
    const columns = [{
      ...
      render: (text, record) => {
        return <div>
          <a href={`#/PartyBuildReport/PartyBuildTask/MeetingTask/PartyBranchesDetail?id=${record.id}`} 
    ...
    return (<div className='secondOrganizationFromMeet' style={{padding:'22px 26px 50px',}}>
        ...
        <Button style={{ width:80,borderRadius:12,margin:'20px 0' }} >导出Excel</Button>
        ...
        <Row type='flex' justify='center' style={{marginTop:10,marginRight:60}}>
          ...
          href={'#/PartyBuildReport/PartyBuildTask/MeetingTask/FirstOrganization'}
          >返回
          </Button>
        </Row>
    </div>)
