...
export default class SpecialPoint extends Component {
  ...
  componentDidMount() {
    ...
    //获取专项积分发放管理列表初始数据
    getService(API_PREFIX + 'services/awardpoint/specialTreasureTask/taskList/1/10', data => {
  ...
  deleteData = () => {
    ...
    postService(API_PREFIX + 'services/awardpoint/specialTreasureTask/delete/task', body, data => {
      if (data.retCode === 1) {
  ...
  stopTask = (record) => {
    ...
    getService(API_PREFIX + `services/awardpoint/specialTreasureTask/updateJobIdStatus/${record.id}/` + status, data => {
      if (data.retCode === 1) {
        ...
        getService(API_PREFIX + `services/awardpoint/specialTreasureTask/taskList/${this.state.currentPage}/${this.state.pageSize}`, data => {
          if (data.retCode === 1) {
            this.setState({
              pointData: data.root.list,
              totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    getService(API_PREFIX + `services/awardpoint/specialTreasureTask/taskList/${current}/${pageSize}`, data => {
      if (data.status === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
  ...
  render() {
    ...
    let createPowers = powers && powers['20011.21602.001'];
    let updatePowers = powers && powers['20011.21602.002'];
    let readPowers = powers && powers['20011.21602.003'];
    let deletePowers = powers && powers['20011.21602.004'];
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          ...
          //         :null
          //       }
          //       {record.enable?<Divider type="vertical" />:null}
          ...
          return <div>
            ...
            style={{ display: 'inline-block' }} >编辑</a>
              :record.expired===1?
              <Popconfirm title="确定停止任务吗?" onConfirm={() => this.stopTask(record)}>
                <a className='operation' >停止任务</a></Popconfirm>
              :record.expired===2?
                <span>
                  <Popconfirm title="确定开启任务吗?" onConfirm={() => this.stopTask(record)}>
    ...
    return <Spin spinning={this.state.loading}><div className="custom-table">
      <div className="custom-table-btn">
        {createPowers?(<Button className="queryBtn" type="primary" onClick={() => { location.hash = '#/PointManagement/SpecialPoint/AddTask'; }}>
          添加
        </Button>):null}
        ...
        {deletePowers?(
                  <Popconfirm title="确定删除所选项吗？" onConfirm={this.deleteData} visible={this.state.visible_M} onVisibleChange={this.handleVisibleChange} onCancel={this.Popcancel}
                  okText="确定" cancelText="取消">
                  <Button type="primary" className="deleteBtn" disabled={selectedRowKeys.length > 0 ? false : true}>
                    删除
                  </Button>
                </Popconfirm>
        ):null}
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      if (this.props.flag) {
        ...
        postService(API_PREFIX + 'services/awardpoint/treasureRecharge/update/TreasureRecharge', values, data => {
          if (data.retCode === 1) {
        ...
        postService(API_PREFIX + 'services/awardpoint/treasureRecharge/add/TreasureRecharge', values, data => {
          if (data.retCode === 1) {
            message.success(data.retMsg);
            this.props.refreshData();
            this.props.closeModal();
