...
export default class EditTask extends Component {
  ...
  componentWillMount() {
    ...
    //获得编辑时的数据
    getService(API_PREFIX + `services/awardpoint/specialTreasureTask/single/${id}`, data => {
      if(data.retCode === 1){
        this.setState({
          editTaskData: data.root,
          operateType:data.root.operateType
        });
      }
    });
  }
  ...
  render(){
    ...
    const tabs = [{
      ...
      content: <EditBasicInfo
        editTaskData={this.state.editTaskData} operateType={operateType} />,
    }, {
      tab:'设置发放对象' ,
      key:'1',
      content: <EditGrantee
