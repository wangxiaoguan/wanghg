...
export default class CategoryManagement extends Component {
  ...
  componentDidMount() {
    //获取类别管理列表初始数据
    getService(API_PREFIX + 'services/awardpoint/specialTreasureTask/typeList/1/10', data => {
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(API_PREFIX + `services/awardpoint/specialTreasureTask/typeList/${this.state.currentPage}/${this.state.pageSize}`, data => {
      if (data.retCode === 1) {
        this.setState({
          categoryData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    getService(API_PREFIX + `services/awardpoint/specialTreasureTask/typeList/${current}/${pageSize}`, data => {
      if (data.status === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          categoryData: data.root.list,
...
class CategoryForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      let ratio = JSON.stringify(values['ratio']).indexOf(".")
      let lengthR = JSON.stringify(values['ratio']).length
      if(ratio>0){
        if(ratio===1){
          if(lengthR-ratio>=2){
            values['ratio']=parseFloat(JSON.stringify(values['ratio']).substring(0,4))
          }else{
            values['ratio']=parseFloat(JSON.stringify(values['ratio']).substring(0,3))
          }
        }else if(ratio===2){
          if(lengthR-ratio>=2){
            values['ratio']=parseFloat(JSON.stringify(values['ratio']).substring(0,5))
          }else{
            values['ratio']=parseFloat(JSON.stringify(values['ratio']).substring(0,4))
          }
        }
      ...
      postService(API_PREFIX + 'services/awardpoint/specialTreasureTask/update/type', values, data => {
        if (data.retCode === 1) {
