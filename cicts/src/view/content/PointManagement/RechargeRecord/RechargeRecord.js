...
export default class RechargeRecord extends Component {
  ...
  componentDidMount() {
    ...
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + 'services/awardpoint/userTreasureRecharge/list/1/10', data => {
  ...
  exportExcel = () => {
    let path = API_PREFIX + 'services/awardpoint/userTreasureRecharge/export' + '?' + `${this.state.values}`;
  ...
  onPageChange = (current, pageSize) => {
    getService(API_PREFIX + `services/awardpoint/userTreasureRecharge/list/${current}/${pageSize}?${this.state.values}`, data => {
      if (data.status === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
  ...
  render() {
    ...
    let readPowers = powers && powers['20011.21604.003'];
    let exportPowers = powers && powers['20011.21604.202'];  
...
class SearchForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      getService(API_PREFIX + `services/awardpoint/userTreasureRecharge/list/1/${this.props.pageSize}?${qFilter}`, data => {
        if (data.retCode === 1) {
    ...
    return (
      <Form onSubmit={this.handleSubmit} >
        <Row style={{marginTop:'17px'}}>
          <Col span={8} pull={1}>
            <FormItem {...formItemLayout1} label="姓名" maxLength="60" style={{marginLeft:'-25px'}} >
