...
export default class RechargePoint extends Component {
  ...
  componentDidMount(){
    ...
    //获取积分充值配置列表初始数据
    getService(API_PREFIX + 'services/awardpoint/treasureRecharge/list/1/10',data=>{
  ...
  deleteData = ()=>{
    ...
    postService(API_PREFIX+'services/awardpoint/treasureRecharge/remove/TreasureRecharge', body, data => {
      if(data.retCode === 1){
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(API_PREFIX + `services/awardpoint/treasureRecharge/list/${this.state.currentPage}/${this.state.pageSize}`, data => {
      if(data.retCode === 1){
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) =>{
    getService(API_PREFIX + `services/awardpoint/treasureRecharge/list/${current}/${pageSize}`,data =>{
  ...
  render() {
    ...
    const rowSelection = {
      ...
      let createPowers = powers && powers['20011.21603.001'];
      let updatePowers = powers && powers['20011.21603.002'];
      let readPowers = powers && powers['20011.21603.003'];
      let deletePowers = powers && powers['20011.21603.004'];
      ...
      <Form onSubmit={this.handleSubmit} >
        ...
        <FormItem {...formItemLayout} label="所需金额" maxLength="11">
          {getFieldDecorator('money',
            ...
            <InputNumber min={0.01} max={99999} step={1} formatter={limitDecimals} parser={limitDecimals}/>
          )}
        </FormItem>
