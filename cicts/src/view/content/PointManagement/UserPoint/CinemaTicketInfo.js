...
export default class CinemaTicketInfo extends Component {
  ...
  componentDidMount() {
    //获取用户积分管理列表初始数据
    getService(API_PREFIX + `services/web/point/info/getSpecialPointList/1/10?Q=userId=${this.state.userid}&Q=pointType=3${this.state.values}`, data => {
  ...
  exportExcel = () => {
    ...
    let path = API_PREFIX + `services/web/point/info/export/getSpecialPointList?Q=userId=${this.state.userid}&Q=pointType=3${this.state.values}`;
    exportExcelService1(path,'电影票积分变更详情');
  }
  ...
  //刷新列表展示数据
  refreshData = () => {
    getService(API_PREFIX + `services/web/point/info/getSpecialPointList/${this.state.currentPage}/${this.state.pageSize}?Q=userId=${this.state.userid}&Q=pointType=3${this.state.values}`, data => {
      if (data.status === 1) {
        this.setState({
          pointData: data.root.list,
          totalNum: data.root.totalNum,
  ...
  onPageChange = (current, pageSize) => {
    ...
    if(!this.state.values) {
      str = 'Q=userid_I_EQ=' + this.state.userid + '&Q=typeid_I_EQ=3'
    }else {
      str = this.state.values
    }
    getService(API_PREFIX + `services/web/point/info/getSpecialPointList/${current}/${pageSize}?Q=userId=${this.state.userid}&Q=pointType=3${this.state.values}`, data => {
      if (data.status === 1) {
        this.setState({
          totalNum: data.root.totalNum,
          pointData: data.root.list,
...
class SearchForm extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, fieldsValue) => {
      ...
      getService(API_PREFIX + `services/web/point/info/getSpecialPointList/1/${this.props.pageSize}?Q=userId=${this.state.userid}&Q=pointType=3${this.state.queryFilter}`, data => {
        if (data.status === 1) {
