...
class MoviePiaoChange extends Component{
    constructor(props){
        super(props);
        this.state={
            loading: true,
    ...
    componentDidMount(){
        this.setState({loading:false})
         ...
         //变更数据类型
         let MoviePiaoTypeData=[];
         getService(
          API_PREFIX +'services/awardpoint/filmChangeRecord/changeType',data=>{
            if(data.retCode===1){
              MoviePiaoTypeData=data.root.object;
              this.getMoviePiaoTypeData(MoviePiaoTypeData);
              this.setState({isOrNotOptions:MoviePiaoTypeData,loading: false})
            }else{
              message.error(data.retMsg);
              this.setState({ loading: false });
            }
  ...
  getMoviePiaoTypeData(data){
    ...
    render(){
          const search=[
            ...
            let exportExcelPower = powers && powers['20011.21607.202']
            return(
            <div>
            <TableAndSearch
            search={search}
            columns={columns}
            exportBtn={exportExcelPower?{order:2,url:'services/awardpoint/filmChangeRecord/export',type:'电影票积分变更'}:null}
            type='platformPointDetail'
            url='services/awardpoint/filmChangeRecord/list'
            />
            </div>
        )
    }
...
export default MoviePiaoChange;