...
class CurrencyIntegralChange extends Component{
    constructor(props){
        super(props);
        this.state={
            loading: true,
    ...
    componentDidMount(){
      ...
      //变更数据类型
      let CurrencyIntegralTypeDate=[];
      getService(
        API_PREFIX +'services/awardpoint/generalChangeRecord/changeType',data=>{
          if(data.retCode===1){
            CurrencyIntegralTypeDate=data.root.object;
            this.getCurrencyIntegralTypeData(CurrencyIntegralTypeDate);
            this.setState({isOrNotOptions:CurrencyIntegralTypeDate,loading: false})
          }else{
            message.error(data.retMsg);
            this.setState({ loading: false });
          }
    ...
    dealDepartmentData(data) {
      ...
      //处理变更类型的数据
  getCurrencyIntegralTypeData(data){
    ...
    render(){
          const search=[
            ...
            let exportExcelPower = powers && powers['20011.21608.202']
            return(
            <div>
            <TableAndSearch
            search={search}
            columns={columns}
            exportBtn={exportExcelPower?{order:2,url:'services/awardpoint/generalChangeRecord/export',type:'通用积分变更'}:null}
            type='platformPointDetail'
            url='services/awardpoint/generalChangeRecord/list'
            />
            </div>
        )
    }
}
export default CurrencyIntegralChange;