...
class BirthdayCouponChange extends Component{
    constructor(props){
        super(props);
        this.state={
            loading: true,
    ...
    componentDidMount(){
      ...
      //变更类型数据
      let BirthchangeTypeData=[];
      getService(
        API_PREFIX +'services/awardpoint/birthdayChangeRecord/changeType',data=>{
          if(data.retCode===1){
            BirthchangeTypeData=data.root.object;
            this.getBirthchangeTypeData(BirthchangeTypeData);
            this.setState({isOrNotOptions:BirthchangeTypeData,loading: false})
          }else{
            message.error(data.retMsg);
            this.setState({ loading: false });
          }
  ...
  getBirthchangeTypeData(data){
    ...
    render(){
          const search=[
            ...
            let exportExcelPower = powers && powers['20011.21605.202']
            return(
            <div>
            <TableAndSearch
            search={search}
            columns={columns}
            exportBtn={exportExcelPower?{order:2,url:'services/awardpoint/birthdayChangeRecord/export',type:'生日劵积分变更'}:null}
            type='platformPointDetail'
            url='services/awardpoint/birthdayChangeRecord/list'
            />
            </div>
        )
    }
...
export default BirthdayCouponChange;