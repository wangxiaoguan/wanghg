...
export default class Add extends Component {
  ...
  getData = () => {
    ...
    getService(API_PREFIX + `services/web/company/org/orgList/get?${isAll}&${haveUsers}`,data=>{
      if (data.status === 1) {
        this.setState({
          dpRootId:data.root.object&&data.root.object[0].id,
        });
  ...
  render() {
    return <EventAndInfoAdd key={this.state.update} type="event" steps={this.state.steps} style="detail" id={{ id: this.state.id }} belonged="apply" save={`/EventManagement/Apply/List?id=${this.state.activeKey}`} dpRootId={this.state.dpRootId}/>;
  }
}
