...
getData=()=>{
  ...
  getService(API_PREFIX + `services/web/activity/voting/reportActivityTitle/${this.state.activeId}`, data => {
    if (data.status === 1) {
...
getBumenData=()=>{
  ...
  getService(API_PREFIX + `services/web/activity/voting/reportTwoOrgRate/${this.state.tenantId}/${this.state.activeId}`, data => {
    if (data.status === 1) {
 ...
 getQiyeData=()=>{
  ...
  getService(API_PREFIX + `services/web/activity/voting/reportCompanyRate/${this.state.activeId}`, data => {
    if (data.status === 1) {
    ...
    getNextData=(orgId)=>{
      if(orgId==this.state.oldId){
        ...
        this.setState({bumenArr:[]},()=>{
          getService(API_PREFIX + `services/web/activity/voting/reportThreeOrgRate/${orgId}/${this.state.activeId}`, data => {
            if (data.status === 1) {               
                    data.root.object.length>0&&data.root.object.map((item,index)=>{
                      item.orgId=item.orgId?item.orgId:"-1";
                      item.average=item.average?item.average:"0";
     ...
     const {bumenArr}=this.state;
      return (
        <Spin spinning={this.state.loading}>
          <div>
            {rate&&rate.length>0&&type==1?<Collapse onChange={()=>this.callback(type)} destroyInactivePanel accordion>
                {rate.map((item,index)=>{
                 return  <Panel showArrow={false} header={<div  onClick={()=>this.getNextData(item.orgId)}><span>{index+1}、{item.orgName}(总人数:{item.orgUserNum},参与人数:{item.joinNum})</span><Progress strokeLinecap="square"  strokeColor={{
                  from: '#108ee9',
                  to: '#108ee9',
                }}  percent={Number((item.joinRate).replace("%",""))}  format={percent => `${item.joinRate}`} strokeWidth={15}/></div>}
        ...
        return (<div style={{marginLeft:"20px"}}><span>{list.orgName}(总人数:{list.orgUserNum},参与人数:{list.joinNum})</span><Progress  strokeColor={{
          from: '#108ee9',
          to: '#108ee9',
        }} strokeLinecap="square" percent={Number((list.joinRate).replace("%",""))} format={percent => `${list.joinRate}`} strokeWidth={15} /></div>) }):<div style={{color:"#999",fontSize:"12px"}}>暂无数据</div>}
        </Panel> })}             
