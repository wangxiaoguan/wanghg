...
export default class VoteInfo extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      total:"",
      companyData:[],/////企业数据
      activityName:JSON.parse(window.sessionStorage.getItem("activityName")) || "投票标题",
    };
  }
                    ...
                    item.optionInfos.map((items, list) => (
                     <Col>
                      ...
                      >
                       ...
                       {items.url?<img src={ossViewPath + items.url} alt="" style={{width:" 100px",height:"100px",marginRight:"30px",marginBottom:'20px'}}  />:null}
                       {items.urlDescribe?items.urlDescribe:null}
                       </Col>
                    ))}
                </Row>
              </Checkbox.Group>
  ...
  render() {
    ...
    return <div className="CarouselDetail1">
      ...
      {GetQueryString(location.hash, ['activeKey']).activeKey==1?
       ...
       exportBtn={{order:2 ,url:`services/web/activity/voting/reportAllJoinDetail`,type:'投票信息', label:'导出'}}
       goBackBtn={{ order: 1, url: `#/EventManagement/Vote/List?id=${this.state.activeKey}`,label:'返回' }}
       />:
       <TableAndSearch rowkey="userId" columns={columns} url={`services/web/activity/question/getAllJoinDetail`}  urlfilter={`Q=activityId=${activityId}`} search={search}
