...
export default class Detail extends Component {
  ...
  getData1=()=>{
    let promise = new Promise((pass,fail)=>{
      getService(API_PREFIX +`services/web/activity/question/getById/${this.state.id}`,data=>{
  ...
  render() {
    return <EventAndInfoAdd key={this.state.update} type="event" steps={this.state.steps} style="detail" id={{ id: this.state.id }} save={`/EventManagement/Questionnaire/List?id=${this.state.activeKey}`} belonged="questionnaire" dpRootId={this.state.dpRootId}/>;
  }
}
const infoOption = [
