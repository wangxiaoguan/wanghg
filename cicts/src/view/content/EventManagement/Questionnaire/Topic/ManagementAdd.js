...
export default class QuestionsConfiguration extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.isEdit) {
      getService(
        API_PREFIX + `services/web/activity/questiontitle/getById/${this.state.id}`,
        data => {
          if (data.status === 1) {
          ...
          console.log("wwwwwwwwwwwwwwwwwwwww",inputValue);
            ...
            this.setState(
              ...
              () => {
                ...
                console.log("===.",value.charLimit)
                this.props.form.setFieldsValue({
                  minOption: Number(value.minOption),
                   maxOption: Number(value.maxOption),
                   charLimit: Number(value.charLimit),
                });
 ...
 // console.log("value",value);
  ...
  render() {
    const { inputOption, inputNum, FieldTypeOption, optionType, questionTypeValue, optionTypeValue,titleArr,isEdit } = this.state;
    const {
      form: { getFieldDecorator },
    } = this.props;
    const formItemLayout = { labelCol: { span: 4 }, wrapperCol: { span: 15 } };
    console.log("questionTypeValue====",titleArr,isEdit);
    return (
