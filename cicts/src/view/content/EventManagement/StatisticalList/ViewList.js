...
export default class ViewList extends Component {
  ...
  render() {
    ...
    let urlName='';
    if(this.props.location.pathname==="/EventManagement/Apply/ViewList"){
      urlName = '#/EventManagement/Apply/List'+`?id=${sessionStorage.getItem('TabsKey')}`;
    }
    return <TableAndSearch key={updateKey} columns={columns} 
      url={'services/web/activity/enrolment/getAllViewDetail'} 
      search={search} 
      // deleteBtn={{ order: 1, url: 'services/activity/activityStatistic/remove/viewUser' }} 
      exportBtn={{ order: 2, url: 'services/web/activity/enrolment/exportViewList', type: '浏览列表', label: '导出浏览列表'}} 
