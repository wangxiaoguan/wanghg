...
class CommentList extends Component {
  ...
  handleChange = (e, record) => {
    ...
    postService(API_PREFIX + `services/web/activity/enrolment/setGoodComment/${commentId}/${status}`, body, data => {
      console.log("普通评论/精彩评论", data);
      if (data.status == 1) {
        message.success('操作成功');
        // history.go()
        this.props.getData(API_PREFIX + `services/web/activity/enrolment/getAllCommentDetail/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}`);
      }
    });
  }
  //排序发生改变
  handleOrderChange = (e) => {
    this.setState({ order: e.target.value }, () => {
      ...
      this.props.getData(API_PREFIX + `services/web/activity/enrolment/getAllCommentDetail/${this.props.pageData.currentPage}/${this.props.pageData.pageSize}?${this.props.pageData.query}&Q=sort=${this.state.order}`);
    });
  }
  ...
  //得到1级评论详情
  getMainDetail = e => {
    getService(API_PREFIX + `services/web/activity/enrolment/getByCommentId/${e.id}`, data => {
      if (data.status === 1) {
  ...
  render() {
    ...
    return <div><TableAndSearch columns={columns}
      ...
      url={`services/web/activity/enrolment/getAllCommentDetail`}
