        ...
        const columns = [
            ...
            },
            {
                ...
                render(text, record) {
                    return (
                        <a onClick={() => {
                            location.hash = `/TradeManager/ChairmanBox/ReportBox/Detail?id=${record.id}&type=ChairReport`;
                        }}>查看详情</a>
                    );
                },
            },
        ];
        const showExport = powers['20007.21707.202'];//导出
        const exportBtnProps = {
            order: 3,
            url: 'services/web/union/chairman/report/export',
            type: '主席信箱上报信件',
        };
        return (
            <div>
                <TableAndSearch
                    search={search}
                    url={'services/web/union/chairman/report/getList'}
                    columns={columns}
