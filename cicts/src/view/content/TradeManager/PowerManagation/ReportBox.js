        ...
        const columns = [
            ...
            },
            {
                ...
                render(text, record) {
                    return (
                        <a onClick={() => {
                            location.hash = `/TradeManager/PowerManagation/ReportBox/Detail?id=${record.id}&type=PowerReport`;
                        }}>查看详情</a>
                    );
                },
            },
        ];
        const showExport = powers['20007.21709.202'];//导出
        const exportBtnProps = {
            order: 3,
            url: 'services/web/union/protectRight/report/export',
            type: '维权管理上传信件',
        };
        return (
            <div>
                <TableAndSearch
                    search={search}
                    url={'services/web/union/protectRight/report/getList'}
                    columns={columns}
