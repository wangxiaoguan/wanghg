        ...
        const columns = [
            ...
            },
            {
                ...
                render(text, record) {
                    return (
                        <a onClick={() => {
                            location.hash = `/TradeManager/PowerManagation/CommonBox/Detail?id=${record.id}&type=PowerCommon`;
                        }}>查看详情</a>
                    );
                },
            },
        ];
        const showExport = powers['20007.21708.202'];//导出
        const exportBtnProps = {
            order: 3,
            url: 'services/web/union/protectRight/common/export',
            type: '维权管理普通信件',
        };
        return (
            <div>
                <TableAndSearch
                    search={search}
                    url={'services/web/union/protectRight/common/getList'}
                    columns={columns}
