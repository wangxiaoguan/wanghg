        ...
        const columns = [
            ...
            },
            {
                ...
                render(text, record) {
                    return (
                        <a onClick={() => {
                            location.hash = `/TradeManager/DifficultyHelp/CommonBox/Detail?id=${record.id}&type=DiffcultyCommon`;
                        }}>查看详情</a>
                    );
                },
            },
        ];
        const showExport = powers['20007.21710.202'];//导出
        const exportBtnProps = {
            order: 3,
            url: 'services/web/union/troubleHelp/common/export',
            type: '困难帮扶普通信件',
        };
        return (
            <div>
                <TableAndSearch
                    search={search}
                    url={'services/web/union/troubleHelp/common/getList'}
                    columns={columns}
