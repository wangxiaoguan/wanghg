        ...
        const columns = [
            ...
            },
            {
                ...
                render(text, record) {
                    return (
                        <a onClick={() => {
                            location.hash = `/TradeManager/DifficultyHelp/ReportBox/Detail?id=${record.id}&type=DiffcultyReport`;
                        }}>查看详情</a>
                    );
                },
            },
        ];
        const showExport = powers['20007.21711.202'];//导出
        const exportBtnProps = {
            order: 3,
            url: 'services/web/union/troubleHelp/report/export',
            type: '困难帮扶上报信件',
        };
        return (
            <div>
                <TableAndSearch
                    search={search}
                    url={'services/web/union/troubleHelp/report/getList'}
                    columns={columns}
