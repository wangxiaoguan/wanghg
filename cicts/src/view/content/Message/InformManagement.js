...
class InformManagement extends Component {
        ...
        render: (text, record) => {
          return (
            <div className="tdControl">
              <a
                disabled={!(this.props.powers && this.props.powers['20008.23003.002'])} 
                style={{ display: record.status === 0 ? "" : "none" }} onClick={() => {
                this.setState({ replyId: record.id, targetId: record.targetId },this.refs.replyModal.show);
              }}>处理</a>
            </div>
          );
        },
  ...
  requestData() {
    ...
    getService(API_PREFIX + `services/message/report/reportList/get/${this.state.current}/${this.state.PageSize}${searchStr}`, (res) => {
    ...
    return (
      <Spin spinning={this.state.loading}>
        <div className="IdeaManagement">
                  ...
                  <Select allowClear={true}>
                    {
                      Object.keys(REASON).map((key) => {
                        return <Option key={key} value={REASON[key]}>{REASON[key]}</Option>
                      })
                    }
                  </Select>)
              }
          ...
          <InformReply ref="replyModal" id={this.state.replyId} targetId={this.state.targetId} replyHandler={() => {
            this.requestData();
          }} />
        </div>
      </Spin>
