...
import { message, Divider, Spin, Modal, Form, Input, InputNumber, Button,Radio,Select  } from 'antd';
...
@Form.create()
export default class SecondClassifyList extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
      ...
      parentId: GetQueryString(location.hash, ['id']).id || '-1',
      classifyOption: [], //新建和编辑的时候，一级分类下拉框数据
    };
  }

  componentWillMount() {
    localStorage.setItem("selectedRowKeys", '');
    getService(API_PREFIX + `services/web/mall/product/category/getTopCategory`, res => {
      if (res.status == 1) {
        this.setState({classifyOption: res.root.object || []})
      }
    })
  }
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (data, record) => {
          return <div>
            {/* <a onClick={() => { location.hash = `GoodsManagetion/secondClassify?id=${record.id}` }} className='operation'>下级</a>
    ...
    return <Spin spinning={this.state.loading}>
      <TableAndSearch columns={columns} url={'services/web/mall/product/category/getList/'}
        search={search} addBtn={{ order: 1, OnEvent: this.addClassify }}
        deleteBtn={{ order: 5, url: 'services/web/mall/product/category/delete', }}
        goBackBtn={{order: 6, label: '返回', url: ''}}
      urlfilter={`Q=parentId=${this.state.parentId}`}
      />
        ...
        <Form onSubmit={this.handleSubmit}>
          <Form.Item {...formItemLayout} label='一级分类'>
            {getFieldDecorator('parentId', {
              ...
              initialValue: this.state.editInfo.parentId || this.state.parentId || '',
            })(
              <Select>
                {
                  this.state.classifyOption && this.state.classifyOption.map(_ => {
                    return (<Select.Option key={_.id} value={_.id}>{_.name}</Select.Option>);
                  })
                }
              </Select>
            )}
          </Form.Item>
          <Form.Item {...formItemLayout} label='二级级分类'>
